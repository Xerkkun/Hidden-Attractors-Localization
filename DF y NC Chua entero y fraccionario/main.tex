...
\subsection{Ruta auxiliar Machado/FDF para generar semillas armónicas}
...
La derivación explícita que sigue corresponde a la subfamilia centrada y real
\(\sigma_0=0\), \(\theta=0\), que permite presentar la ecuación de amplitud
en forma escalar. La ruta computacional vigente no se limita a este caso:
evalúa la forma sesgada \(N_{\phi}^{\rm L}(A,\sigma_0)\) y, para cada
\((A,\sigma_0,\omega,\mu)\), aplica la fase de cierre Machado/FDF mediante
\begin{equation}
\label{eq:machado_operational_closure_ns}
\begin{aligned}
N_{\phi}^{\rm M}(A,\sigma_0;\mu,\theta_\ell)
&=\exp\!\left(\mu\operatorname{Log}
N_{\phi}^{\rm L}(A,\sigma_0)\right)e^{i\theta_\ell},\\
\theta_\ell&=\frac{2\pi\ell}{24},\qquad \ell=0,\ldots,23,\\
R_{\rm M}(A,\sigma_0,\omega;\mu,\theta_\ell)
&=1+\chi W_q(i\omega)N_{\phi}^{\rm M}(A,\sigma_0;\mu,\theta_\ell).
\end{aligned}
\end{equation}
Se conservan para el ordenamiento los valores de \(\theta_\ell\) que
minimizan \(\lvert R_{\rm M}\rvert\) en cada punto evaluado; el indicador de
armónicos superiores \(\rho_H\) no cambia por esta rotación global de fase.
Este \(\theta_\ell\) pertenece al cierre FDF y se registra en la tabla del
candidato. No debe confundirse con \(\phi\), la fase orbital que se recorre
después para reconstruir distintas condiciones iniciales a partir de una
misma semilla armónica.
...
Una vez obtenida una semilla por la línea Lur'e o por la línea Machado/FDF, se
usa la ganancia armónica correspondiente \(k\) para escribir
\begin{equation}
\label{eq:delta_ns}
...
\item fijar los parámetros de \eqref{eq:danca_params_ns}, el orden $q$, el paso $h$, el integrador y la política de memoria;
...
\item integrar cada semilla durante un transitorio y una ventana de observación causal con la matriz EFORK/ABM de la Subsección~\ref{subsec:busqueda_sistematica_ns}, descartando las señales periódicas persistentes sólo después del transitorio;
\item conservar únicamente semillas \path{nonperiodic_post_transient} en la celda primaria EFORK sin truncamiento e integrar \eqref{eq:homotopy_ns} sobre una malla $0=\varepsilon_0<\varepsilon_1<\cdots<\varepsilon_N=1$;
...
\item al llegar a $\varepsilon=1$, integrar el sistema físico y aplicar las pruebas de vecindades de los equilibrios y de cuencas requeridas para discutir ocultedad.
\end{enumerate}
...
ejecución bajo el contrato descrito enseguida. La tabla de síntesis del caso
se conserva para mantener la estructura comparativa del reporte y marca como
pendientes los campos que dependen de esas corridas.
...
En la ruta Machado/FDF, la búsqueda numérica recorre
\begin{equation}
\begin{gathered}
A\in[0.25,20],\qquad \sigma_0\in[-8,8],\\
\omega\in[10^{-4},10],\qquad \ell=0,\ldots,23,\\
\mu\in\{0.25,0.5,0.75,1,1.25,\\
\hphantom{\mu\in\{}1.5,2,3,4,5\},\\
\theta_\ell=\dfrac{2\pi\ell}{24}.
\end{gathered}
\end{equation}
Estas son las \(24\) fases \(\theta_\ell\) de
\eqref{eq:machado_operational_closure_ns}. En particular, la malla de
\(\theta_\ell\) participa en la evaluación del residuo FDF; no se usa sólo
como fase de reconstrucción de la condición inicial. Una vez elegido un
punto \((A,\sigma_0,\omega,\mu,\theta_\ell)\), la reconstrucción recorre por
separado fases orbitales \(\phi\), conservando en los artefactos ambos
parámetros para mantener trazabilidad matemática.
...
Tabla~\ref{tab:chua_frac_ns_results_full} conserva la estructura de resultados
del caso fraccionario no suave. Los parámetros y contratos metodológicos son
los vigentes; los valores dinámicos se completarán únicamente desde las
corridas postransitorias actuales.
...
\setlength{\tabcolsep}{3pt}
\renewcommand{\arraystretch}{1.12}
\caption{Resumen de resultados y campos de validación pendientes para el Chua fraccionario no suave con \(q=0.9998\).}
...
Parámetros del sistema &
Definición del caso no suave &
\(\alpha=8.4562,\ \beta=12.0732,\ \gamma=0.0052,\ m_0=-0.1768,\ m_1=-1.1468\) &
Sistema fraccionario no suave evaluado en la búsqueda actual. \\
...
Rutas de semilla &
DF/FDF de primer armónico &
Lur'e clásica/sesgada y Machado/FDF &
Las dos líneas se exploran antes de aplicar filtros dinámicos. \\

Integrador principal &
EFORK-3 corregido en C &
\(K_3=h^q f(X_n+a_{31}K_1+a_{32}K_2)\) &
La decisión primaria se toma con EFORK sin truncamiento de historia. \\

Matriz de sensibilidad &
EFORK y ABM, con y sin truncamiento &
\(\begin{array}{c}
\mathrm{EFORK}_{\rm FH},\ \mathrm{EFORK}_{L_m=8}\\
\mathrm{ABM}_{\rm FH},\ \mathrm{ABM}_{L_m=8}
\end{array}\) &
ABM se usa como comparación; la promoción de candidatos corresponde a EFORK. \\

Tramo de evaluación &
Criba de periodicidad posterior al transitorio &
\(T_{\rm tr}=120,\ T_{\rm obs}=120,\ h=0.01\) &
La periodicidad no se decide con la parte transitoria de la trayectoria. \\

Semillas armónicas &
Búsqueda y reconstrucción DF/FDF &
\textbf{Pendiente de promoción} &
Se reportarán identificador, método, parámetros DF/FDF y condición inicial desde la corrida vigente. \\

Tres mejores candidatos &
Filtro dinámico y ranking &
\textbf{Pendiente de promoción} &
La tabla final distinguirá la ruta generadora y el contrato EFORK aplicado. \\

Continuación causal &
Homotopía con historia transportada &
\textbf{Pendiente} &
Sólo semillas no periódicas después del transitorio podrán continuar. \\

Diagnóstico dinámico &
Trayectoria, FFT/PSD y Lyapunov &
\textbf{Pendiente} &
Las métricas se incorporarán sólo para sobrevivientes de continuación. \\

Robustez &
Variaciones de paso, horizonte y memoria &
\textbf{Pendiente} &
Se compararán EFORK sin truncamiento y la sensibilidad bajo ventana finita. \\

Sondeos de ocultedad &
Vecindades de todos los equilibrios &
\textbf{Pendiente} &
No se usará \path{hidden_verified} sin completar el contrato de vecindades. \\
...
\subsection{Ruta auxiliar Machado/FDF para generar semillas con no linealidad \texorpdfstring{\(\arctan\)}{arctan}}

Además de la función descriptiva clásica, se considera una extensión auxiliar
basada en la propuesta de Machado para ampliar el conjunto de semillas del
sistema fraccionario de Chua con no linealidad \(\arctan\). Esta ruta se
plantea en paralelo con la línea de Lur'e: no la sustituye ni reemplaza la
condición de Nyquist, sino que deforma la ganancia armónica equivalente del
bloque no lineal antes de la misma validación causal.
...
\subsubsection{Relación operativa entre las dos líneas}

Al explorar conjuntamente la ruta clásica y la extensión Machado/FDF se
...
Una vez construida una semilla mediante cualquiera de las dos rutas, la
descomposición
\begin{equation}
...
\qquad \varepsilon\in[0,1].
\end{equation}
La continuación que se implementa con EFORK es causal en Caputo. Por ello,
...
\item seleccionar la ruta clásica o Machado/FDF y resolver sus ecuaciones de cierre y amplitud;
\item construir las semillas asociadas a la ruta seleccionada;
...
En esta sección se reportan los resultados numéricos de la búsqueda y continuación de candidatos a atractores ocultos en el sistema de Chua fraccionario no suave con $q=0.9998$. Toda la integración temporal y el transporte homotópico se realizaron mediante el integrador EFORK de paso fijo $h=0.02$, utilizando tanto la rama con historia completa de Caputo como la rama de ventana de memoria finita ($L_m=20.0$).

\subsection{Selección y Promoción de Candidatos}
Siguiendo la metodología oficial unificada de 10 etapas, se generó un conjunto inicial de semillas analíticas utilizando los balances armónicos de las familias Lur'e (clásica centrada y sesgada) y la extensión Machado/FDF. Tras la aplicación del pre-chequeo suave (donde no se rechazaron semillas por periodicidad pre-continuación), se ejecutó la continuación numérica mediante el parámetro de homotopía $\lambda\in[0,1]$.

Los supervivientes dinámicos del filtro post-continuación (que descartó colapsos a equilibrio y divergencias) se ordenaron de acuerdo con la amplitud observada en el régimen estacionario, el residual de balance armónico y la medida de distorsión $\rho_H$. La Tabla~\ref{tab:candidatos_frac_chua} detalla las especificaciones analíticas y métricas de los tres candidatos principales promovidos para la rama de historia completa.

\begin{table*}[htbp]
\centering
\caption{Especificaciones y métricas dinámicas de los tres candidatos principales de Chua fraccionario ($q=0.9998$).}
\label{tab:candidatos_frac_chua}
\small
\begin{tabular}{clcccccccc}
\toprule
\textbf{Rango} & \textbf{ID Candidato} & \textbf{Familia DF} & \textbf{Amplitud $A$} & \textbf{Frec. $\omega_0$} & \textbf{Ganancia $k$} & \textbf{Semilla inicial $\bm{x}(0)$} & \textbf{Rango obsv. $x$} & \textbf{FFT Peak} & \textbf{Entropía PSD} \\
\midrule
1 & \texttt{centered\_machado\_mu2} & Machado centrada ($\mu=2.0$) & $2.6284$ & $2.0403$ & $0.2100$ & $(2.6284,\; 0.1664,\; -3.7555)$ & $9.1405$ & $0.28$ & $0.1297$ \\
2 & \texttt{centered\_classical} & Lur'e clásica centrada & $5.8518$ & $2.0403$ & $0.2100$ & $(5.8518,\; 0.3704,\; -8.3610)$ & $9.1401$ & $0.28$ & $0.1294$ \\
3 & \texttt{centered\_machado\_mu4} & Machado centrada ($\mu=4.0$) & $1.7149$ & $2.0403$ & $0.2100$ & $(1.7149,\; 0.1086,\; -2.4503)$ & $9.1399$ & $0.28$ & $0.1295$ \\
\bottomrule
\end{tabular}
\end{table*}

Es importante destacar que el candidato de rango 1 proviene del plano Machado con $\mu=2.0$, mientras que el de rango 2 corresponde al balance de Lur'e clásico centrado. Ambos confluyen en una frecuencia fundamental idéntica ($\omega_0\approx 2.0403$) y una ganancia $k\approx 0.2100$, lo cual confirma la consistencia del mapa de linealización y valida la homotopía fraccionaria de transporte.

\subsection{Análisis de Espacio de Fases y Series Temporales}
Para el atractor candidato mejor clasificado (\texttt{centered\_machado\_mu2}), se obtuvieron los retratos de fase 3D, las proyecciones en planos bidimensionales, la evolución de variables temporales y el espectro de potencia de la señal $x(t)$. Las Figuras~\ref{fig:danca_frac_phase_3d}, \ref{fig:danca_frac_projections}, \ref{fig:danca_frac_time_series} y \ref{fig:danca_frac_spectrum_x} muestran estas gráficas iniciales generadas directamente a partir del solver nativo de historia completa.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.74\columnwidth]{../version_2/library_figures/by_report/df_nc_chua/pdf/danca_frac_phase_3d.pdf}
\caption{Espacio de fases 3D para el candidato \texttt{centered\_machado\_mu2} en Chua fraccionario ($q=0.9998$) integrado con C EFORK-3.}
\label{fig:danca_frac_phase_3d}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.96\columnwidth]{../version_2/library_figures/by_report/df_nc_chua/pdf/danca_frac_projections.pdf}
\caption{Proyecciones geométricas en los planos $xy$, $xz$ y $yz$ para el atractor candidato fraccionario.}
\label{fig:danca_frac_projections}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.90\columnwidth]{../version_2/library_figures/by_report/df_nc_chua/pdf/danca_frac_time_series.pdf}
\caption{Series temporales de las variables $x(t)$, $y(t)$ y $z(t)$ del candidato en régimen de estado transitorio final y estable.}
\label{fig:danca_frac_time_series}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\columnwidth]{../version_2/library_figures/by_report/df_nc_chua/pdf/danca_frac_spectrum_x.pdf}
\caption{Espectro de potencia FFT de la señal $x(t)$ que muestra el carácter caótico y de banda estrecha del atractor.}
\label{fig:danca_frac_spectrum_x}
\end{figure}
