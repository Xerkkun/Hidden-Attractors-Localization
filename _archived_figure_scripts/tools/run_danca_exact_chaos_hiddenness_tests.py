...
Q = 0.9998
H = 0.01
CASE_ID = "danca2017_exact"
DEFAULT_X0 = np.array(
    [3.039383584794975, -0.2416862069577155, -6.873467365218827],
    dtype=float,
)
...
VAL_DIR = (
    ...
    / "danca2017_exact_system_validation"
)
FIG_DIR = ROOT / "DF y NC Chua entero y fraccionario copy" / "Figs"
REFERENCE_TRAJECTORY = ROOT / "danca_ref_trajectory.csv"
...
def _savefig(name: str, profile: str) -> None:
    ...
    for ext in ("png", "pdf"):
        plt.savefig(FIG_DIR / f"{profiled}.{ext}", dpi=220, bbox_inches="tight")
...
def run_lyapunov(profile: Profile, output_dir: Path) -> dict[str, object]:
    ...
    method_specs = [
        ...
        },
        {
            ...
            "extra": {
                "t_clone": profile.cloned_t_clone,
                "k_blocks": profile.cloned_k_blocks,
                "delta": 1.0e-4,
                "orders": [Q, Q, Q],
...
def _plot_lyapunov_convergence(path: Path, profile: str) -> None:
    ...
    axes[-1].set_xlabel("t")
    fig.tight_layout()
    _savefig("danca_exact_fig12_lyapunov_two_methods", profile)
    plt.close(fig)
...
def _plot_hiddenness_spheres(
    ...
    _savefig("danca_exact_fig13_hiddenness_spherical_probes", profile)
    plt.close(fig)
...
def _plot_hiddenness_heatmap(rows: list[dict[str, object]], profile: str) -> None:
    ...
    fig.tight_layout()
    _savefig("danca_exact_fig14_hiddenness_contact_heatmap", profile)
    plt.close(fig)
...
def main() -> None:
    ...
    parser.add_argument("--only", choices=("all", "lyapunov", "hiddenness"), default="all")
