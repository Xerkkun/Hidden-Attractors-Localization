#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Test de ocultedad extendido para los equilibrios E+ y E- (excluyendo E0):
   m1=-1.1468, m0=-0.1768, branch=1, c=+2.776

Protocolo:  Muestreo ball (volumen completo) alrededor de E+ y E-.
...
OUTDIR    = (Path("outputs/biased_saturation_search_q09998_corrected/hiddenness")
             / (PREFIX + "_ep_em_extended"))
OUTDIR.mkdir(parents=True, exist_ok=True)

# Plan de radios para E+ y E-: (radio, n_muestras)
RADIUS_PLAN = [
    (1e-5,   150),
    (3e-5,   200),
    (1e-4,   300),
...
def plot_heatmap(records, outdir):
    ...
    ax.set_title("Fraccion TARGET hits (Solo E+ y E-, Radios Extendidos)\n"
...
def main():
    ...
    log("=" * 80, logf)
    log(f"TEST EXTENDIDO COMPLETO (E+ / E-): m1={M1}, m0={M0}, c={C_BIAS:+.3f}", logf)
    log(f"  Radios: {RADII}", logf)
    log(f"  N/radio: {SAMPLES_PER_R}", logf)
    log(f"  Total/equilibrio: {sum(SAMPLES_PER_R)}   Total (2 eq): {sum(SAMPLES_PER_R)*2}", logf)
    ...
    # Filtrar solo E+ y E-
    eq_targets = {name: pt for name, pt in equilibria.items() if name in ("E+", "E-")}
    log(f"\n  Evaluando unicamente los equilibrios: {list(eq_targets.keys())}", logf)
    ...
    t0_global   = time.time()
    total_probes = sum(SAMPLES_PER_R) * len(eq_targets)
    probe_count = 0
    ...
    for eq_idx, (eq_name, eq_pt) in list(enumerate(eq_targets.items())):
    ...
    log(f"\n{'='*80}", logf)
    log("TABLA RESUMEN COMPLETA POR EQUILIBRIO x RADIO (E+ y E-)", logf)
