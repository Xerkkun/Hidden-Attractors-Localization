...
Esta versión del manuscrito retira los resultados numéricos de ejecuciones anteriores. La función descriptiva clásica de Lur'e (centrada y sesgada) se utiliza exclusivamente como generadora de semillas; la selección de candidatos y sus pruebas se poblarán a partir de las corridas del flujo básico bajo el contrato descrito enseguida. La tabla de síntesis del caso se conserva para mantener la estructura comparativa del reporte y marca como pendientes los campos correspondientes.

\subsubsection{Protocolo de búsqueda y criba básica}

Para la corrida promovible se implementó una ruta basada en la función descriptiva clásica de Lur'e en sus variantes centrada y sesgada. Esta etapa sólo propone semillas iniciales aproximadas.
...
Antes de ejecutar una continuación, cada semilla reconstruida se integra utilizando el solver EFORK-3 con historia completa de Caputo y paso fijo $h=0.01$ (sin truncamiento de memoria). El intervalo \(0\leq t\leq T_{\rm tr}\) se descarta como transitorio, y la observación del régimen dinámico se realiza sobre \(T_{\rm tr}\leq t\leq T_{\rm tr}+T_{\rm obs}\), fijando:
\[
T_{\rm tr}=120,\qquad T_{\rm obs}=120,\qquad h=0.01.
\]

Las semillas que colapsan a equilibrios o divergen en esta etapa son descartadas. Las semillas supervivientes que exhiben un comportamiento acotado no trivial avanzan a la continuación homotópica ($\lambda \in [0, 1]$). Al alcanzar el sistema objetivo ($\lambda = 1$), se realiza la verificación final de ocultedad mediante las pruebas de vecindades de los equilibrios: se integra el sistema partiendo de múltiples condiciones iniciales muestreadas en bolas de radio pequeño alrededor de cada equilibrio para determinar si alguna de ellas converge al atractor caótico candidato.
...
Rutas de semilla &
DF de primer armónico &
Lur'e clásica (centrada y sesgada) &
La línea clásica se explora antes de aplicar filtros dinámicos. \\
...
Integración y sensibilidad &
EFORK con historia completa &
EFORK sin truncamiento &
La integración y promoción se realizan con EFORK sin truncamiento. \\
...
La búsqueda y validación del atractor se realiza integrando el sistema con el solver EFORK-3 en C utilizando historia completa. A continuación se presentan las secciones listas para recibir las figuras y datos de la corrida:

\begin{enumerate}
    \item \textbf{Candidato de la ruta clásica}: Parámetros armónicos seleccionados: $\omega_0$, ganancia $k$ y amplitud $A_0$. Semilla inicial reconstruida $\bm{x}(0) = (x_0, y_0, z_0)$.
    \item \textbf{Trayectoria del atractor caótico}: Retrato de fases en el espacio 3D y proyecciones geométricas 2D del atractor superviviente de la continuación causal a $\varepsilon=1$.
    \item \textbf{Sondeos de ocultedad (vecindades de equilibrios)}: Resultados de la simulación partiendo de esferas/bolas de vecindad alrededor de los equilibrios inestables. Se registrará la proporción de trayectorias que convergen al atractor y el veredicto final.
\end{enumerate}

\subsection{Resultados: Chua fraccionario con no linealidad $\arctan$ ($q=0.99$)}

De forma análoga al caso anterior, los resultados de la simulación y criba para el sistema con no linealidad de tipo $\arctan$ se estructuran bajo el flujo básico de criba:

\begin{enumerate}
    \item \textbf{Candidato y Semilla}: Parámetros obtenidos mediante Nyquist y función descriptiva exacta del bloque Lur'e $\psi(\sigma) = a_2\arctan(\rho\sigma)$.
    \item \textbf{Trayectoria final del atractor}: Espacio de fases 3D para el sistema objetivo tras la continuación paramétrica completa.
    \item \textbf{Pruebas de ocultedad}: Análisis operacional de cuencas de atracción alrededor de todos los equilibrios calculados, determinando si el atractor es clasificado operacionalmente como oculto o autoexcitado.
\end{enumerate}
