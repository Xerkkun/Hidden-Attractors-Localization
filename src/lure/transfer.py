...
def W_eval(omega: float | np.ndarray, q: float, transfer_mode: str, P: np.ndarray, b: np.ndarray, r: np.ndarray, transfer_convention: str = "standard") -> complex | np.ndarray:
    ...
    # Vectorized evaluation using diagonalization:
    # W(s) = r^T (s*I - P)^(-1) * b = sum_j (c_left[j] * b_right[j]) / (s - evals[j])
    try:
...
def W_precompute_spectral(
    ...
    Instead of calling ``np.linalg.eig(P)`` on every frequency grid point,
    callers can compute the decomposition once and reuse it via
    :func:`W_eval_from_cache`.

    Returns
    -------
    dict with keys:
        ``evals``     : eigenvalues of P, shape (n,)
        ``residues``  : r^T V * (V^{-1} b), shape (n,)  — partial-fraction residues
        ``convention_sign`` : +1 for ``"standard"``, -1 for ``"opposite_sign"``
        ``fallback``  : True if eigendecomposition failed (use slow path)
    """
    try:
        evals, V = np.linalg.eig(P)
        V_inv = np.linalg.inv(V)
...
def W_eval_from_cache(
    ...
    omega : float or np.ndarray
        Frequency / frequencies (must be positive and finite).
    q : float
        Fractional order (0 < q ≤ 1).
    transfer_mode : str
        ``"integer"`` or ``"fractional"``.
    cache : dict
        Output of :func:`W_precompute_spectral`.

    Returns
    -------
    complex or np.ndarray
        Transfer function value(s).
    """
