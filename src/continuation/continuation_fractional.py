...
def run_fractional_continuation(
    ...
    For integrator="abm", this is redirected to run_fractional_continuation_abm_monolithic,
    which implements pure-Python monolithic history transport to avoid derivative causality
    violation at stage boundaries.
    ...
    early_stop_reason, history_policy, carry_derivative_history, eta_boundary_policy.
    """
...
def run_fractional_continuation_abm_monolithic(
    ...
    Carries the exact derivative history ('fhist') across eta stages.
    Historical derivative values (f_j) computed under a previous eta stage
    are preserved verbatim and never re-evaluated under a new eta stage.
    """
    from scipy.special import gamma
    from ..integrators.abm import eval_rhs
    ...
    # We evaluate prehistory using the first stage's eta deform system.
    p0 = system.P + k_gain * np.outer(system.b, system.r)
    ...
    # Track index in the big arrays
    curr_n = K - 1  # the index of the last completed step
    ...
    for stage_idx, eta in enumerate(lambda_values):
        ...
        # We will run nsteps_tr transient steps, then nsteps_kp keep steps.
        ...
        # End of the stage integration (or early stopped)
        if early_stopped:
            ...
            # Successfully finished the entire stage
