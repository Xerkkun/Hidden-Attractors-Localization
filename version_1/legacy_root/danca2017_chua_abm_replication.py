        ...
        out.append(
            {
                ...
                "hidden_verified": data.get("hidden_verified", False),
...
def write_comparison_report(cfg: DancaChuaConfig, outdir: Path) -> Path:
    ...
    lines.append("")
    lines.append("| group | candidate | method | q | current status | hidden_verified |")
    lines.append("|---|---|---|---:|---|---:|")
    for row in project_rows:
        lines.append(
            "| {candidate_group} | {best_candidate} | {method} | {q} | {status} | {hidden_verified} |".format(
    ...
    lines.append("")
    lines.append("Danca Fig. 3 and the project candidates use the same non-smooth Chua parameters and q, but not the same localization route.  Danca reports a trial-and-error hidden attractor observed with ABM; the project candidates were generated by Lure/Machado harmonic seeds and validated with EFORK.  A candidate is not promoted to `hidden_verified` unless equilibrium-neighborhood controls fail to reproduce the target under the tested contract.")
    report.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return report
