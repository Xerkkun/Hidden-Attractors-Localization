...
from validation.python.phase_f_closure import assess_phase_f_closure
from validation.python.run_phase_F_closure_assessment import run
...
def test_phase_f_runner_writes_closure_artifacts(tmp_path: Path) -> None:
    output = tmp_path / "phase_F_closure"
    run(output)
    assert (output / SUMMARY.name).is_file()
...
def test_routes_a_and_b_are_assessed_not_discarded_or_promoted() -> None:
    ...
    assert route_a["f4_method_control"]["status"] == "separate_internal_control_only_not_published_validated"
