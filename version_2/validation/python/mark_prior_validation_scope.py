#!/usr/bin/env python3
"""Record the evidence boundary of stage summaries from an earlier run."""
...
def mark_summaries(validation_root: Path, contract_path: Path) -> list[str]:
    ...
    for stage in contract["stages"]:
        ...
        scope = {
            ...
            "current_contract": "validation/00_manifest/source_configs/unified_caputo_protocol.json",
            "current_contract_applied": False,
            "note": (
                "This promoted summary predates the current unified Caputo protocol. "
                "It remains a historical record and must not be treated "
                "as a completed run of the frozen validation contract."
            ),
        }
