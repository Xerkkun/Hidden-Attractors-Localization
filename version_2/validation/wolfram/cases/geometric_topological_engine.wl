...
check[name_, condition_] := Module[{ok = TrueQ[condition]},
  AppendTo[tests, MakeTest[name, ok]];
...
ExportJSON[
  ...
  <|
    "system_id" -> systemID,
    "validation_scope" -> "symbolic_geometric_topological_identities_and_case_residuals",
    "report_input_used" -> False,
    "tests" -> tests,
    "passed" -> And @@ (TrueQ[Lookup[#, "passed"]] & /@ tests),
    "files" -> <|
      "text_evidence" -> FileNameTake[evidencePath]
    |>,
    "evidence_boundary" -> "No attraction, chaos, basin topology, Conley index, or hiddenness claim is established."
  |>
];
ExitFromTests[tests];
