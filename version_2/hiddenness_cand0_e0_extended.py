#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Test de ocultedad extendido para E0:
   m1=-1.1468, m0=-0.1768, branch=1, c=+2.776

Protocolo:  Muestreo ball (volumen completo) alrededor del equilibrio E0.
...
# Plan de radios para E0: (radio, n_muestras)
RADIUS_PLAN = [
    (1e-5,   150),
    (3e-5,   200),
    (1e-4,   300),
...
def plot_heatmap(records, outdir):
    ...
    data       = np.full((1, len(RADII)), np.nan)
    count_data = np.zeros((1, len(RADII)), dtype=int)
    ...
    for rec in records:
        ...
        for jj, rad in enumerate(RADII):
            if abs(rec["radius"] - rad) < rad * 0.01:
                j = jj; break
        if j >= 0:
            data[0, j]       = rec["TARGET"] / max(rec["samples"], 1)
            count_data[0, j] = rec["samples"]
    ...
    ax.set_title("Fraccion TARGET hits (Solo E0, Radios Extendidos)\n"
    ...
    for j in range(len(RADII)):
        if not np.isnan(data[0, j]):
            n_txt = count_data[0, j]
            t_txt = int(data[0, j] * n_txt)
            txt   = f"{data[0,j]:.2f}\n({t_txt}/{n_txt})"
            col   = "white" if data[0,j] > 0.55 else "black"
            ax.text(j, 0, txt, ha="center", va="center", fontsize=7, color=col)
...
def main():
    ...
    log("=" * 80, logf)
    log(f"TEST EXTENDIDO PARA E0 (Multiprocessing): m1={M1}, m0={M0}, c={C_BIAS:+.3f}", logf)
    log(f"  Radios: {RADII}", logf)
    log(f"  N/radio: {SAMPLES_PER_R}", logf)
    log(f"  Total integraciones: {sum(SAMPLES_PER_R)}", logf)
    ...
    eq_name = "E0"
    eq_pt = equilibria[eq_name]
    log(f"  Punto de inicio E0: {np.array2string(eq_pt, precision=6)}", logf)

    # Determinación de cores para usar
    n_workers = min(12, multiprocessing.cpu_count() - 2)
    n_workers = max(1, n_workers)
    log(f"  Iniciando Pool de multiprocessing con {n_workers} workers...", logf)

    all_records: List[Dict[str, Any]] = []
    all_runs:    List[Dict[str, Any]] = []
    t0_global   = time.time()
    total_probes = sum(SAMPLES_PER_R)
    probe_count = 0
    ...
    for r_idx, (radius, n_samples) in enumerate(RADIUS_PLAN):
        t0_radio = time.time()
        log(f"\n  [{eq_name}]  radio={radius:.0e}  n={n_samples}", logf)
    ...
    log(f"\n{'='*80}", logf)
    log("TABLA RESUMEN POR RADIO (Solo E0)", logf)
    log(f"{'Radio':<10} {'N':>6} {'TARGET':>8} {'EQ':>6} {'OTHER':>7} {'DIV':>5} {'Fraccion':>10}", logf)
    log("-"*70, logf)
    for rec in all_records:
        frac = rec["TARGET"] / max(rec["samples"], 1)
        log(f"{rec['radius']:<10.0e} {rec['samples']:>6d} {rec['TARGET']:>8d} "
