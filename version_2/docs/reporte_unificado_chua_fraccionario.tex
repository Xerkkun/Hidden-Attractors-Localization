...
\graphicspath{{assets/figures/chua_fractional_report/}{assets/figures/chua_integer_q1/}{assets/figures/efork3_validation/}}
...
\newcommand{\maybeincludegraphics}[2][]{%
  \IfFileExists{assets/figures/chua_fractional_report/#2}{%
    ...
    \IfFileExists{assets/figures/chua_integer_q1/#2}{%
      ...
      \IfFileExists{assets/figures/efork3_validation/#2}{%
        \includegraphics[#1]{assets/figures/efork3_validation/#2}%
      }{%
...
\begin{enumerate}
  \item La función descriptiva, Nyquist y Machado/FDF generan semillas. No
...
\codepath{hidden_attractors/seed_generation/} & Semillas por función descriptiva, Nyquist, Lur'e y Machado/FDF. \\
...
\begin{figure}[H]
\centering
\maybeincludegraphics[width=0.80\textwidth]{chaos_lorenz_phase_3d.png}
...
\begin{figure}[H]
\centering
\maybeincludegraphics[width=0.88\textwidth]{chaos_lorenz_time_series.png}
...
\centering
\maybeincludegraphics[width=\linewidth]{chaos_lorenz_poincare.png}
\end{minipage}\hfill
\begin{minipage}{0.49\textwidth}
\centering
\maybeincludegraphics[width=\linewidth]{chaos_lorenz_spectrum.png}
\end{minipage}
...
\begin{figure}[H]
\centering
\maybeincludegraphics[width=0.78\textwidth]{matignon_complex_plane.png}
...
\begin{figure}[H]
\centering
\maybeincludegraphics[width=0.94\textwidth]{chua_nonlinearity_piecewise_vs_arctan.png}
...
\begin{enumerate}
  ...
  \(\operatorname{Im}W_q(\ii\omega)=0\), con
  \[
  W_q(\ii\omega)=r^\top\left((\ii\omega)^qI-P\right)^{-1}b.
  \]
  \item En el cruce se toma \(k=N(A)=1/\operatorname{Re}W_q(\ii\omega)\),
  usando la convención normalizada del reporte. La implementación histórica
  equivalente usa \(r^\top(P-(\ii\omega)^qI)^{-1}b\) y por eso guarda el mismo
  cierre con signo opuesto en la transferencia.
  \item Se resuelve \(N(A)=k\), se construye
  \(P+bkr^\top\), se toma el autovector asociado a
  \((\ii\omega)^q\), se normaliza con \(r^\top v=1\), y la condición inicial
  modal se toma como
  \[
...
\begin{figure}[H]
\centering
\maybeincludegraphics[width=0.78\textwidth]{chua_nonsmooth_typical_attractor_3d.png}
...
\begin{figure}[H]
\centering
\maybeincludegraphics[width=0.78\textwidth]{chua_nonsmooth_df_continuation_3d.png}
...
\begin{figure}[H]
\centering
\maybeincludegraphics[width=0.78\textwidth]{chua_arctan_typical_attractor_3d.png}
\caption{Chua arctan: atractor 3D integrado desde la semilla DF entera calculada por la librería. Parámetros tipo Wu2023: \(a_1=0.4\), \(a_2=-1.5585\), \(\rho=1\).}
\end{figure}

\begin{figure}[H]
\centering
\maybeincludegraphics[width=0.86\textwidth]{chua_arctan_typical_time_series.png}
\caption{Chua arctan: series temporales post-transitorio de la corrida típica usada para el atractor 3D. La gráfica permite revisar acotamiento, escalas relativas y aperiodicidad visual; no reemplaza Lyapunov, Poincaré ni análisis de cuencas.}
\end{figure}

\begin{figure}[H]
\centering
\maybeincludegraphics[width=0.78\textwidth]{chua_arctan_df_continuation_3d.png}
\caption{Chua arctan: continuación ilustrativa desde la linealización armónica hasta la realimentación suave \(a_2\arctan(\rho\sigma)\).}
\end{figure}
...
W_q(\ii\omega)=r^\top\left((\ii\omega)^qI-P\right)^{-1}b.
\]
Para \(q=1\), esto coincide con la transferencia ordinaria \(W_1(\ii\omega)\).
Para \(0<q<1\), la librería usa la convención local
\(\lambda=(\ii\omega)^q\). Esta operación es una herramienta de semillas, no
una equivalencia exacta con una órbita periódica de Caputo.

La función descriptiva de la saturación residual
\(\psi(\sigma)=(m_0-m_1)\operatorname{sat}(\sigma)\) es
\begin{equation}
N(A)=
\begin{cases}
m_0-m_1, & A\le 1,\\[4pt]
...
\]
La rama de Machado/FDF aplica una deformación de ganancia para búsqueda de
semillas. La librería la trata como heurística auxiliar y no como validación
matemática de ciclos fraccionarios.
...
\begin{figure}[H]
\centering
\maybeincludegraphics[width=0.82\textwidth]{convergence_errors.png}
...
\codepath{find_lure_harmonic_seed} & \codepath{seed_generation/} & \(1-N(A)W_q(\ii\omega)=0\). & Baseline entero y residuos complejos. & Semilla heurística. \\
\codepath{machado_describing_function} & \codepath{seed_generation/} & Deformación FDF de ganancia. & Contrato local; referencia Machado. & Exploración auxiliar. \\
...
\begin{figure}[H]
\centering
\maybeincludegraphics[width=0.76\textwidth]{example_quickstart_equilibria.png}
...
\begin{figure}[H]
\centering
\maybeincludegraphics[width=0.78\textwidth]{example_candidate_reading.png}
...
\begin{figure}[H]
\centering
\maybeincludegraphics[width=0.95\textwidth]{example_minimal_protocol_flow.png}
...
\begin{figure}[H]
\centering
\maybeincludegraphics[width=0.78\textwidth]{example_api_layers.png}
...
\begin{figure}[H]
\centering
\maybeincludegraphics[width=0.82\textwidth]{fig01_nyquist_df.png}
...
\begin{figure}[H]
\centering
\maybeincludegraphics[width=0.80\textwidth]{fig01c_transfer_real_imag.png}
...
\begin{figure}[H]
\centering
\maybeincludegraphics[width=0.82\textwidth]{fig02d_continuation_story.png}
...
\begin{figure}[H]
\centering
\maybeincludegraphics[width=0.82\textwidth]{fig05b_hiddenness_overview.png}
...
\begin{minipage}{0.49\textwidth}
\centering
\maybeincludegraphics[width=\linewidth]{fig11a_fft_x.png}
\end{minipage}\hfill
\begin{minipage}{0.49\textwidth}
\centering
\maybeincludegraphics[width=\linewidth]{fig11d_psd_x.png}
\end{minipage}
...
\begin{figure}[H]
\centering
\maybeincludegraphics[width=0.78\textwidth]{phase_space_3d.png}
...
\begin{figure}[H]
\centering
\maybeincludegraphics[width=0.92\textwidth]{phase_projections.png}
\caption{Galería de análisis: proyecciones de fase. Las proyecciones ayudan a inspeccionar estructura, simetría y escala.}
\end{figure}

\begin{figure}[H]
\centering
\maybeincludegraphics[width=0.92\textwidth]{time_series.png}
...
\begin{figure}[H]
\centering
\maybeincludegraphics[width=0.88\textwidth]{bifurcation_diagram.png}
...
\begin{figure}[H]
\centering
\maybeincludegraphics[width=0.86\textwidth]{robustness_overlay_lure.png}
...
\end{minipage}
\caption{Robustez de candidatos Machado/FDF. Estas figuras son evidencia de persistencia numérica, no prueba de ocultedad.}
\end{figure}
...
La transferencia fraccionaria usa
\[
W_q(\ii\omega)=r^\top\left((\ii\omega)^qI-P\right)^{-1}b.
\]
...
\begin{figure}[H]
\centering
\maybeincludegraphics[width=0.82\textwidth]{danca_abm_fig03.png}
...
\end{minipage}
\caption{Candidatos generados por Lur'e y Machado/FDF. La semejanza geométrica es diagnóstica; la decisión depende de robustez y cuencas.}
\end{figure}
...
La formulación algebraica, los equilibrios, el Jacobiano y el split de Lur'e
están implementados. La guía no afirma reproducción completa ni ocultedad
verificada del caso arctan mientras no exista una rama robusta, referencia
dinámica promovida, comparación publicada y paquete de cuencas completo. Esto
evita convertir un caso implementado en una conclusión científica más fuerte
...
\begin{itemize}
  ...
  archivos de origen.
  \item DF/Nyquist/Machado/FDF son generadores de semillas.
...
\begin{center}
\footnotesize
\begin{longtable}{@{}L{0.32\textwidth}L{0.46\textwidth}L{0.14\textwidth}@{}}
\toprule
Comando & Uso & Estado \\
\midrule
\cmdbreak{hidden-attractors}{run -p chua\_integer} & Ejecuta preset entero. & Usuario \\
\cmdbreak{hidden-attractors}{run -p chua\_fractional} & Ejecuta preset fraccionario no suave. & Usuario \\
\cmdbreak{hidden-attractors}{run -p chua\_arctan} & Ejecuta preset arctan. & Usuario \\
\cmdbreak{hidden-attractors}{init -e chua\_fractional} & Copia YAML editable. & Usuario \\
\begin{tabular}[t]{@{}l@{}}\texttt{hidden-attractors}\\\texttt{inspect-config}\\\texttt{-p chua\_fractional}\end{tabular} & Muestra configuración efectiva. & Usuario \\
\cmdbreak{hidden-attractors}{systems} & Lista sistemas registrados. & Auxiliar \\
\cmdbreak{hidden-attractors}{workflow-requirements} & Explica requerimientos por workflow. & Auxiliar \\
\cmdbreak{hidden-attractors}{protocol} & Ejecuta etapas oficiales del protocolo. & Alfa \\
\cmdbreak{hidden-attractors}{robustness-overlay} & Workflow de robustez. & Alfa \\
\cmdbreak{hidden-attractors}{sphere-controls} & Muestreo de esferas/bolas. & Alfa \\
\cmdbreak{hidden-attractors}{refined-basin} & Refinamiento de cuencas. & Alfa \\
\cmdbreak{hidden-attractors}{strict-target-refinement} & Refinamiento estricto de TARGET. & Alfa \\
\cmdbreak{hidden-attractors}{check-validation} & Revisa contrato de validación. & Auxiliar \\
\bottomrule
\end{longtable}
\end{center}
