...
\begin{enumerate}
  ...
  prueban existencia global de un ciclo ni ocultedad
    prueban existencia global de un ciclo ni ocultedad~\cite{GenesioTesi1993,KuznetsovEtAl2017,Machado2015FDF}.
...
y \codepath{jacobian_arctan}. Este modelo está conectado a Wu et al. 2023~\cite{Wu2023ArctanChua}, pero el reporte conserva su estado como caso
...
\codepath{efork3_caputo_integrate} & \codepath{solvers/} & Runge-Kutta fraccionario EFORK y tablas manufacturadas Ghoreishi-Ghaffari & Referencia y solver. \\
...
\codepath{compute_lyapunov_spectrum} & \codepath{analysis/lyapunov_api.py} & Registro de métodos de Lyapunov para Phase F controles, discrepancias y estado separado, sin promoción automática & sin promoción automática \\
...
\codepath{examples/chua_arctan_wu2023/} & \codepath{01_search.py}, \codepath{02_continue.py}, \codepath{03_verify.py}, \codepath{04_plot.py}, \codepath{run_example.py}, \codepath{reproducibility.yaml} & Ejemplo 2: reproducción bibliográfica Wu2023, rama DF fraccionaria y candidato dinámico c590. \\
\bottomrule
\end{longtable}
\end{center}
...
En v1.0.0, el candidato c590 se promueve solo como evidencia local radius-limited:
radios \(r\le0.3\), \(8400\) pruebas finitas y cero contactos. Esta etiqueta
no es prueba global de cuenca.
...
El resumen vigente usa radios \(10^{-5},10^{-4},10^{-3},10^{-2},0.03,0.1,0.3,1,2\). Para los radios locales hasta \(r=0.3\) registra \(8400\) pruebas finitas y cero contactos alrededor de todos los equilibrios, por lo que el candidato c590 se promueve como \codepath{hiddenness_supported_under_tested_local_radii}. Los contactos aparecen unicamente en los radios macroscopicos extendidos \(r=1\) y \(r=2\), con \(610\) contactos en \(5100\) pruebas; esos radios se conservan como auditoria extendida de cuenca y no como la frontera de la afirmacion local promovida.
...
\caption{Arctan c590: mapa de contactos de vecindad. La afirmacion promovida usa radios locales hasta \(r=0.3\); los contactos en \(r=1\) y \(r=2\) se conservan como auditoria macroscopica extendida.}
\end{figure}
...
Chua fraccionario no suave, reproduccion Danca & \begin{tabular}[t]{@{}l@{}}reproduccion parcial\\no promovida\end{tabular} & Contrato bibliografico y datos extraidos. & Faltan datos publicados para reproduccion completa de trayectoria. \\
...
Chua arctan fraccionario Wu2023 & \begin{tabular}[t]{@{}l@{}}pendiente de\\validación\end{tabular} & \codepath{configs/chua_arctan_wu2023_caputo.json}; ADM local. & No es validacion Caputo completa de ocultedad. \\
Chua arctan c590 & promovido localmente & \codepath{validation/chua_fractional_arctan_c590/}; \(8400\) pruebas sin contactos hasta \(r=0.3\). & Evidencia finita de ocultedad local; radios \(1\) y \(2\) quedan como auditoria extendida. \\
Metodología Lur'e/Caputo & validado como framework & Modelos, API, semillas, integradores, verificacion, manifiestos y reporte. & Alcance restringido a sistemas compatibles con Lur'e escalar. \\
\bottomrule
\end{longtable}
\end{center}
...
\midrule
Chua entero & {\footnotesize \begin{tabular}[t]{@{}l@{}}\codepath{run_example.py}\\\codepath{--steps search}\\continuation\\verification figures\end{tabular}} & \codepath{examples/chua_integer_lure_reference/reproducibility.yaml}. \\
Chua fraccionario no suave & {\footnotesize \begin{tabular}[t]{@{}l@{}}\codepath{run_example.py}\\\codepath{--steps 1 2 3 5}\\wrappers \codepath{01_search.py}--\\\codepath{04_plot.py}\\son\\conveniencia.\end{tabular}} & \codepath{configs/examples/chua_nonsmooth_biased_df_search.yaml}; Danca no reproducible en \codepath{configs/examples/chua_nonsmooth_exact_danca_non_reproducible.yaml}. \\
Chua arctan c590 & {\footnotesize \begin{tabular}[t]{@{}l@{}}\codepath{run_example.py}\\\codepath{--steps published}\\search\\continuation figures;\\\codepath{--all}\\incluye verificacion larga.\end{tabular}} & Wu2023 en \codepath{configs/chua_arctan_wu2023_caputo.json}; c590 promovido con radios locales \(r\le0.3\). \\
\bottomrule
\end{longtable}
\end{center}
...
\cmdbreak{hidden-attractors}{run -p chua\_arctan} & Ejecuta o inspecciona el preset arctan; c590 se promueve solo bajo el contrato local \(r\le0.3\). & Experimental \\
...
\begin{itemize}
  \item \cmdbreak{hidden-attractors-check-}{validation} (legacy/deprecated; reemplazado por \cmdbreak{hidden-attractors}{validate contract})
  \item \cmdbreak{hidden-attractors-}{protocol} (legacy/deprecated; reemplazado por \cmdbreak{hidden-attractors}{protocol})
\end{itemize}
...
Nonlinear Dynamics, 2002, DOI:\@ \href{https://doi.org/10.1023/A:1016592217141}{doi}.
