...
\else
  \hypersetup{
    pdftitle={Reporte maestro de localización de atractores ocultos enteros y fraccionarios},
...
% Las figuras se clasifican una por una. No se redefine globalmente el entorno
% figure: una figura común se conserva en ambas ediciones y una figura digital
% se omite de la guía de impresión de forma explícita.
...
\input{integer_datos_actuales}
\input{integer_mavpd_chaos_generated}
\input{integer_julia_comparison_generated}

\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small Reporte maestro de atractores ocultos}
...
\ifHAFOPedagogy
  \part{Antes: lenguaje y prerrequisitos}
\else
  \part{Fundamentos y alcance}
\fi
\label{part:guia-geometrica-entera}
\input{report_sections/00_alcance_inventario}
\ifHAFOPedagogy
  \input{report_sections/16_programa_estudio_bibliografico}
\fi
\clearpage
\input{report_sections/01_introduccion_antecedentes}
\begingroup
\def\HAFOModelsOnly{1}
\input{report_sections/02_modelos_y_fundamentos}
\endgroup
\begingroup
\def\HAFONoGeoPosterior{1}
\input{report_sections/14_guia_geometrica_topologica_entera}
\endgroup
\begingroup
\def\HAFOFractionalOnly{1}
\input{report_sections/02_modelos_y_fundamentos}
\endgroup
\input{report_sections/15_geometria_topologia_caputo}
\input{report_sections/17_metodos_topologicos_existencia}

\clearpage
\ifHAFOPedagogy
  \part{Durante: construcción de la metodología}
\else
  \part{Métodos de localización}
\fi
\label{part:metodologia}
\input{report_sections/03_lure_transferencia_funcion_descriptiva}
\input{report_sections/12_machado_funcion_descriptiva}
\input{report_sections/04_integracion_continuacion}
\input{report_sections/13_puntos_perpetuos}
\input{report_sections/19_localizacion_geometrico_topologica}
\input{report_sections/05_protocolos_experimentales}
\input{report_sections/21_campana_dinamica_geometrico_topologica}
\input{report_sections/10_propuestas_teoricas}

\clearpage
\part{Casos estudiados}
\label{part:casos}
\input{report_sections/06_resultados}
\input{report_sections/08_enteros_reproducidos}
\input{report_sections/09_fraccionarios_no_chua}
\input{report_sections/20_resultados_geometrico_topologicos_casos}
\input{report_sections/22_resultados_piloto_geometrico_topologico}

\clearpage
\ifHAFOPedagogy
  \part{Después: síntesis y trabajo futuro}
\else
  \part{Síntesis y problemas abiertos}
\fi
\label{part:futuro}
\input{report_sections/11_candidatos_compatibles}
\begingroup
\def\HAFOGeoPosteriorOnly{1}
\input{report_sections/14_guia_geometrica_topologica_entera}
\endgroup
\input{report_sections/07_discusion_conclusiones}
...
\input{report_sections/B_trazabilidad_pruebas}
\input{report_sections/C_validacion_wolfram}
\input{report_sections/D_validacion_maestra}
\input{report_sections/F_wolfram_no_chua}
\input{report_sections/E_atlas_figuras}
\fi
