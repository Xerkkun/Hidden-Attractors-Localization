...
\begin{tikzpicture}[font=\small,>=Latex]
  ...
  \node[anchor=north west,font=\bfseries] at (0.45,5.25)
    {espacio admisible \(X_{\mathrm{adm}}\) (esquema condicional)};
  ...
  \node[Verde,align=center] at (10.70,3.55)
    {cuenca relativa\\\(\mathcal B_{X_{\mathrm{adm}}}(A)\)};
  ...
  \node[anchor=west,fill=white,inner sep=2pt] (slice) at (0.70,2.55)
    {reinicios admisibles \(\iota(\mathbb R^n)\cap X_{\mathrm{adm}}\)};
  ...
  \node[draw=GrisOscuro,rounded corners=2pt,fill=white,align=center] at (7.45,0.60)
    {ocultedad relativa \(\Longrightarrow\) ocultedad de reinicio,
     si el espacio incluye los reinicios próximos al equilibrio};
\end{tikzpicture}%
}
\par\smallskip
\begin{minipage}{0.92\linewidth}
\small\textbf{Cuenca funcional y corte de reinicio.}
La figura supone un espacio admisible invariante con topología relativa y
reinicios próximos al equilibrio incluidos. En ese marco, una bola del corte
de reinicio puede evitar la cuenca mientras una perturbación funcional
admisible cercana sí la alcanza. Es una representación conceptual, condicionada
a construir el espacio y el atractor. En todo \(\mathfrak C\), la
\cref{prop:geofo-full-space-obstruction} impide esa separación.
\end{minipage}
\end{center}
