% Selectores de bloques para la edición de estudio. Sin
% \HAFOGeoBlockSelect se conserva íntegro el orden histórico A--F. Si el
% selector está activo, pueden habilitarse uno o varios bloques mediante
% \HAFOGeoBlockIntroFlow, \HAFOGeoBlockBasins, \HAFOGeoBlockDegree,
% \HAFOGeoBlockLinear, \HAFOGeoBlockPoincare y \HAFOGeoBlockChaos.
\ifdefined\ifHAFOGeoRenderBlock\else
  \newif\ifHAFOGeoRenderBlock
\fi

\ifdefined\HAFOGeoPosteriorOnly\else
\HAFOGeoRenderBlocktrue
\ifdefined\HAFOGeoBlockSelect
  \HAFOGeoRenderBlockfalse
  \ifdefined\HAFOGeoBlockIntroFlow\HAFOGeoRenderBlocktrue\fi
\fi
\ifHAFOGeoRenderBlock
\section{Geometría y topología de la dinámica}
\label{sec:geo-proposito}

La geometría y la topología relacionan las trayectorias con la estructura del
espacio de fases, los conjuntos invariantes y sus cuencas. Para localizar
atractores ocultos, este análisis permite identificar regiones de atracción,
seguir objetos recurrentes mediante mapas de retorno y estudiar la separación
de sus cuencas respecto de los equilibrios. Las preguntas centrales son:
...
\begin{itemize}
  ...
  \item ¿qué parte de una conclusión procede de un teorema y qué parte es
...
Al estudiar estos conceptos se busca:
...
\subsection{Jerarquía de evidencia geométrica}
\label{geo:subsec:jerarquia-evidencia}

Las afirmaciones se clasifican en cuatro niveles para distinguir visualización,
evidencia numérica y demostración.
...
\toprule
Nivel & Resultado & Alcance & Limitación \\
\midrule
...
Considérese una ecuación autónoma de orden entero
\begin{equation}
  \dot x=f(x),\qquad x\in M,
  \label{geo:eq:edo-manifold}
\end{equation}
donde \(M\) es una variedad diferenciable, usualmente \(\R^n\) o el cilindro
\(\R\times S^1\). Su flujo actúa sobre estados puntuales; en un PVI causal
de Caputo, la memoria exige un espacio funcional para formular la evolución.
El lenguaje topológico y diferencial se apoya en
...
que una especificación de «radio \(10^{-3}\)» no se transporta conservando el mismo
número.
...
\end{equation}
La diferencia angular envuelta evita el salto artificial de coordenadas en
\(\pm\pi\).
...
\end{equation}
Esta transformación relaciona la equivalencia afín de las realizaciones D0/T1
con la geometría del muestreo de cuenca.
...
\begin{proposition}[Picard--Lindelöf local]
...
\begin{remark}[Papel de la autonomía en la propiedad de flujo]
...
\input{report_diagrams/entero_01_flujo_invariancia}
...
sí sola no decide cuál alternativa ocurre. En el caso MAVPD, los ciclos
...
\ifdefined\HAFOGeoBlockSelect
  \HAFOGeoRenderBlockfalse
  \ifdefined\HAFOGeoBlockBasins\HAFOGeoRenderBlocktrue\fi
\fi
\ifHAFOGeoRenderBlock
\HAFOStudyNeedspace
...
\subsection{Conjuntos límite después del transitorio}
...
\begin{proof}
Los conjuntos
\(K_s=\overline{\{\varphi_t(x):t\ge s\}}\) son compactos no vacíos y
anidados. Son conexos porque la cola de la órbita es imagen continua del
intervalo conexo \([s,\infty)\), y la clausura conserva conexidad.
La propiedad de intersección de compactos conexos anidados muestra que
\(\bigcap_{s\ge0}K_s\) es no vacía, compacta y conexa. Si
...
\begin{equation}
 ...
 \label{geo:eq:basin}
\end{equation}
Se adopta como atractor asintóticamente estable un compacto invariante
...
\begin{proof}
Sea \(x\in\mathcal B(A)\) y elija una vecindad abierta atractora \(V\)
de \(A\). Como \(A\) es compacto, existe \(\varepsilon>0\) tal que
\(\{z:\operatorname{dist}(z,A)<\varepsilon\}\subset V\).
La convergencia de \(x\) a \(A\) proporciona entonces un tiempo
\(T\) con \(\varphi_T(x)\in V\). La continuidad de
...
\input{report_diagrams/entero_02_cuencas_ocultedad}
...
Sea \(N\) compacto no vacío y supóngase que el campo apunta estrictamente hacia su
...
\]
La intersección anidada de compactos no vacíos es compacta y no vacía.
Como cada \(\varphi_s\) es un homeomorfismo, para \(s\ge0\)
\[
 \varphi_s(A_N)=\bigcap_{t\ge0}\varphi_{t+s}(N)
 =\bigcap_{t\ge s}\varphi_t(N)=A_N;
\]
la última igualdad usa que suprimir un tramo inicial de una familia anidada
no cambia la intersección. Aplicando \(\varphi_{-s}\), la invariancia se
extiende a tiempos negativos.
La atracción significa
\(\sup_{x\in\varphi_t(N)}\operatorname{dist}(x,A_N)\to0\).
Si fallara, existirían \(\varepsilon>0\), \(t_j\to\infty\) y
\(x_j\in\varphi_{t_j}(N)\) con distancia al menos \(\varepsilon\).
Por compacidad de \(N\), una subsucesión converge a \(x_*\). Para cada
\(s\ge0\), eventualmente \(t_j\ge s\), de modo que
\(x_j\in\varphi_s(N)\); al ser este conjunto cerrado,
\(x_*\in\varphi_s(N)\). Luego \(x_*\in A_N\), en contradicción con
la continuidad de la distancia a un conjunto cerrado.
\end{proof}

Un bloque de este tipo alrededor de un candidato demuestra acotación y
existencia de un conjunto invariante. La clasificación de ese conjunto como
ciclo, conjunto caótico o atractor oculto requiere estudiar su dinámica y su
cuenca.
...
\ifdefined\HAFOGeoBlockSelect
  \HAFOGeoRenderBlockfalse
  \ifdefined\HAFOGeoBlockDegree\HAFOGeoRenderBlocktrue\fi
\fi
\ifHAFOGeoRenderBlock
\HAFOStudyNeedspace
...
El grado topológico es un entero que relaciona la existencia de soluciones
con el comportamiento de un mapa sobre la frontera de un dominio. Permanece
constante durante una homotopía admisible, aunque las soluciones individuales
no se conozcan explícitamente \cite{Amster2021MetodosTopologicos}.
...
Se emplean las dos consecuencias siguientes.
...
\begin{proof}[Idea]
La propiedad de localización permite calcular el grado en un subabierto que
contenga todas las preimágenes de \(y\). Si no hay preimágenes en
\(\Omega\), el grado coincide con el del dominio vacío y es cero. Esto
prueba la primera afirmación por contraposición. La segunda es la invariancia
homotópica del grado: la condición de frontera impide que una raíz entre o
salga del dominio durante la deformación.
\end{proof}

Si \(F\in C^1\) y sus raíces en \(\Omega\) son finitas y no degeneradas,
entonces
\begin{equation}
 \deg(F,\Omega,0)=
 \sum_{x_j\in\Omega:\,F(x_j)=0}\operatorname{sgn}\det DF(x_j).
 \label{geo:eq:degree-sum}
\end{equation}
El sumando es el índice local de la raíz.

\input{report_diagrams/entero_03_grado_indice}

\subsection{Ejemplo resuelto: índices de los equilibrios de Chua}

En una región de pendiente \(m\), el Jacobiano es
\[
 J_m=\begin{pmatrix}
 -\alpha(1+m)&\alpha&0\\
 1&-1&1\\
...
\(\det J_m=-\alpha(1+m)(\beta+\gamma)+\alpha\gamma\), y por tanto
\begin{equation}
...
Para Kalman--Fitts, el cálculo simbólico produce
...
\end{equation}
En una carta de \(\Sigma\), sea \(U\) abierto acotado, con \(P\) definido
y continuo en \(\overline U\) y \(P(u)\ne u\) sobre \(\partial U\).
Un grado no nulo de \(F\) en \(U\) prueba que \(P\) tiene un
...
51 niveles aproxima una rama. Para demostrar persistencia por grado se
requiere un dominio común \(U\), continuidad conjunta de
\((\lambda,u)\mapsto P_\lambda(u)\) sobre
\([0,1]\times\overline U\), y que
\begin{equation}
 P_\lambda(u)\ne u
...
menos un punto fijo hasta \(\lambda=1\). La condición
\eqref{geo:eq:kalman-degree-boundary} excluye la pérdida de todas las
soluciones por la frontera y debe verificarse para el continuo de parámetros.
...
perturbación compacta de la identidad y se tienen cotas a priori. Su aplicación
requiere probar compacidad y ausencia de soluciones sobre la frontera
funcional.
...
cuencas a cuencas. En particular,
\ifHAFOBasinMaterial
la ocultedad definida por \eqref{geo:eq:hidden-topological}
\else
la separación topológica entre las vecindades de los equilibrios y la cuenca
\fi
se conserva.
\end{proposition}
...
\ifdefined\HAFOGeoBlockSelect
  \HAFOGeoRenderBlockfalse
  \ifdefined\HAFOGeoBlockLinear\HAFOGeoRenderBlocktrue\fi
\fi
\ifHAFOGeoRenderBlock
\HAFOStudyNeedspace
...
\HAFOCardLinearGeometry

Estos resultados requieren declarar regularidad e
...
\input{report_diagrams/entero_04_variedades_invariantes}
...
Para el polinomio MAVPD
\(p(\lambda)=\lambda^3+a_1\lambda^2+a_2\lambda+a_3\), la condición
\(a_1a_2=a_3\), junto con \(a_1>0\) y \(a_2>0\), permite factorizar
\[
 p(\lambda)=(\lambda+a_1)(\lambda^2+a_2).
\]
Los autovalores son \(-a_1\) y \(\pm\mathrm i\sqrt{a_2}\): el par
es simple, el centro tiene dimensión real dos y la dirección restante es
estable. Para afirmar una bifurcación de Hopf aún deben comprobarse el
cruce transversal al variar el parámetro y la no degeneración del primer
coeficiente de Lyapunov. La igualdad de Routh--Hurwitz, sin estas
verificaciones, identifica una frontera espectral candidata.

\subsection{Alcance caso por caso}

\begin{table}[H]
\centering
\caption{Hipótesis y alcance del teorema de la variedad estable en los casos analizados.}
...
Kalman--Fitts & Si el espectro del origen queda estrictamente en el
...
\(x=\pm1\). Esta formulación relaciona la teoría de topología y geometría
diferencial con los cálculos de divergencia presentados.

\fi
\HAFOGeoRenderBlocktrue
\ifdefined\HAFOGeoBlockSelect
  \HAFOGeoRenderBlockfalse
  \ifdefined\HAFOGeoBlockPoincare\HAFOGeoRenderBlocktrue\fi
\fi
\ifHAFOGeoRenderBlock
\HAFOStudyNeedspace
...
\end{equation}
Bajo \(f\in C^1\), escriba localmente
\(\Sigma=\{x:H(x)=0\}\), con \(H\in C^1\) y
\(n=\nabla H(p)\). La transversalidad equivale a
\(n^{\mathsf T}f(p)\ne0\). La función
\(G(t,x)=H(\varphi_t(x))\) satisface
\(G(T,p)=0\) y \(\partial_tG(T,p)=n^{\mathsf T}f(p)\ne0\).
El teorema de la función implícita produce así un único tiempo
\(\tau(x)\) cercano a \(T\), de clase \(C^1\), con
\(\tau(p)=T\) y \(G(\tau(x),x)=0\).
Al reducir la sección alrededor de \(p\), el teorema del flujo tubular
excluye un segundo cruce inmediato; el arco compacto restante de
\(\gamma\) entre esos cruces queda separado de la sección.
La dependencia continua excluye también retornos intermedios para puntos
próximos. Por ello \(\tau\) es el primer retorno positivo local y define
\begin{equation}
...
\begin{proof}
Elija \(\rho(DP(p))<\kappa_0<\kappa<1\). En dimensión finita existe una
norma adaptada con \(\|DP(p)\|_*<\kappa_0\). Por continuidad de \(DP\),
en una bola convexa suficientemente pequeña de la carta de \(\Sigma\) se
tiene \(\|DP(x)\|_*\le\kappa\). Integrar la derivada sobre el segmento
entre \(x\) e \(y\) da
\(\|P(x)-P(y)\|_*\le\kappa\|x-y\|_*\).
Como \(P(p)=p\), la bola queda en sí misma y
\(\|P^j(x)-p\|_*\le\kappa^j\|x-p\|_*\).
Además, \(\tau(x)\to T\) cuando \(x\to p\), y los tramos de flujo
entre retornos dependen uniformemente de su punto inicial en un intervalo
temporal compacto. Sus distancias a \(\gamma\) tienden por ello a cero.
El flujo tubular extiende esta estabilidad de la sección a una vecindad de
la órbita, permitiendo un cambio de fase.
Si algún autovalor tiene módulo mayor que uno, el teorema de inestabilidad
por linealización para mapas \(C^1\) se aplica a \(P\); su inestabilidad
se transporta por la misma sección a la órbita
\cite{HirschSmaleDevaney2013,Kuznetsov2023Bifurcation}.
\end{proof}
...
\end{equation}
Con \(\Phi(0)=I\), la matriz \(M=\Phi(T)\) es la matriz de monodromía y sus autovalores son los
multiplicadores de Floquet. La teoría de Floquet proporciona, en general con
matrices complejas, la factorización
\begin{equation}
 \Phi(t)=Q(t)e^{tR},\qquad Q(t+T)=Q(t).
 \label{geo:eq:floquet-factorization}
\end{equation}
Una factorización real puede requerir una matriz periódica de periodo
\(2T\). Los multiplicadores siguen siendo los autovalores de la monodromía
en el periodo original \(T\).
...
de \(T\); así \(Mf(p)=f(p)\), de donde resulta el multiplicador \(1\).
Para \(v\in T_p\Sigma\), la regla de la cadena aplicada al retorno da
\[
 DP(p)v=Mv+f(p)D\tau(p)v.
\]
Como \(P(x)\in\Sigma\), el vector de la izquierda es tangente a la
sección. Multiplicando por \(n^{\mathsf T}\) y usando
\(n^{\mathsf T}f(p)\ne0\), se obtiene
\[
 D\tau(p)v=-\frac{n^{\mathsf T}Mv}{n^{\mathsf T}f(p)},\qquad
 DP(p)v=\left(I-\frac{f(p)n^{\mathsf T}}
 {n^{\mathsf T}f(p)}\right)Mv.
\]
El factor entre paréntesis proyecta sobre \(T_p\Sigma\) a lo largo de la
dirección del flujo. En una base que empieza por \(f(p)\) y continúa con
una base de \(T_p\Sigma\), la matriz \(M\) tiene la forma por bloques
\(\left(\begin{smallmatrix}1&*\\0&DP(p)\end{smallmatrix}\right)\).
Su polinomio característico es, por tanto,
\((\lambda-1)\det(\lambda I_{n-1}-DP(p))\), lo que prueba la
coincidencia con multiplicidades.
\end{proof}

\input{report_diagrams/entero_05_poincare_floquet}
...
\end{equation}
Se usa \(T_f\) para el parámetro del filtro y \(T\) para el periodo de la
...
geométricamente los multiplicadores del ciclo corredor estable
y la órbita separadora inestable.

\subsection{Interpretación geométrica de los casos analizados}

\begin{table}[H]
\centering
\caption{Aplicaciones y requisitos del análisis de Poincaré.}
\label{geo:tab:poincare-cases}
\small
\begin{tabularx}{\textwidth}{@{}L{0.17\textwidth}YY@{}}
\toprule
Caso & Comportamiento observado & Verificación geométrica \\
\midrule
...
En Chua, la falta de diferenciabilidad de \(f\) en \(x=\pm1\) exige una
formulación de Floquet por tramos. Con cruces transversales
...
\subsection{Alcance dimensional del teorema de Poincaré--Bendixson}
...
caso adecuado para estudiar cuencas y separatrices, pero no para establecer
inicialmente una herradura de flujo.
...
\ifdefined\HAFOGeoBlockSelect
  \HAFOGeoRenderBlockfalse
  \ifdefined\HAFOGeoBlockChaos\HAFOGeoRenderBlocktrue\fi
\fi
\ifHAFOGeoRenderBlock
\HAFOStudyNeedspace
...
Una intersección homoclínica transversal proporciona un mecanismo de dinámica
simbólica bajo las hipótesis de Smale--Birkhoff. Su construcción relaciona
variedades invariantes, recurrencia y caos
\cite{HirschSmaleDevaney2013,DatserisParlitz2022Nonlinear}.
...
\begin{proposition}[Teorema de Smale--Birkhoff]
...
visitadas. Su aplicación exige verificar esa construcción global en el sistema
analizado; el teorema fija con precisión las hipótesis necesarias.

\input{report_diagrams/entero_06_homoclinica_herradura}
...
\end{equation}
porque \(h_{\mathrm{top}}(P^k|_\Lambda)=\log2\). Esta cota relaciona de manera
demostrable la geometría de variedades, la codificación simbólica y la complejidad
...
\subsection{Nociones no equivalentes de caos}
...
\subsection{Análisis homoclínico de Chua y MAVPD}

El estudio de un mecanismo homoclínico se organiza en las siguientes etapas:
...
que el candidato caótico local constituye un caso inicial adecuado para
desarrollar el protocolo.

\section{Comparación geométrica de los cuatro casos}
\label{sec:geo-four-cases}

\begin{table}[H]
\centering
\caption{Estructuras geométricas de los cuatro sistemas.}
\label{geo:tab:four-cases-map}
\small
\begin{tabularx}{\textwidth}{@{}L{0.16\textwidth}Y Y@{}}
\toprule
Caso & Objetos geométricos & Problema global \\
\midrule
...
\subsection{Reformulación geométrica de la ocultedad}

La metodología de función descriptiva, puntos perpetuos o mapa de conmutación
localiza una semilla. El análisis geométrico complementario identifica después:
\begin{equation}
 \text{semilla}
 \longrightarrow
 \omega(x_0)
...
o un índice topológico puede producir semillas nuevas y complementar los
métodos algebraicos analizados.

\subsection{Fronteras suaves, fractales y de Wada}
...
tres destinos coexistentes. En Kalman--Fitts se identifican el origen y el
ciclo; los estados sin destino clasificado no constituyen una tercera cuenca
con la que establecer la propiedad de Wada.

\fi

% El material acumulado se conserva en la monografía digital completa. La guía
% de estudio usa los ejercicios distribuidos junto a cada bloque matemático.
...
\paragraph{Pista.}
La evaluación de 51 valores de \(\lambda\) no cubre el continuo
\([0,1]\times\partial U\). La verificación requiere controlar los tiempos de
retorno y excluir tangencias con la sección.
...
\paragraph{Criterio de validación.}
El término «herradura» queda reservado para una intersección transversal
encerrada o una cobertura equivalente. Una distancia pequeña con ángulo mal
...
La defensa debe separar: definición; hipótesis del teorema; evidencia numérica
disponible; conclusión realmente autorizada; y un contraejemplo que muestre por
...
\ifdefined\HAFOGeoBlockSelect\else
\section{Problemas de profundización e investigación}
\label{sec:geo-study-plan}

\HAFOCardFuture

\subsection{Problemas de investigación y requisitos}
...
\toprule
Proyecto & Caso de referencia & Resultado mínimo & Condición de validación rigurosa \\
\midrule
...
\subsection{Criterios para formular conclusiones geométricas}

Toda conclusión geométrica debe especificar explícitamente:
...
los puntos perpetuos. Estos métodos proporcionan condiciones iniciales en una
región del espacio de fases. El flujo determina la órbita; el conjunto
...
topológico demostrado. La separación de estos niveles relaciona los sistemas
analizados con la teoría geométrica de ecuaciones diferenciales.
\end{hallazgo}
\fi
\fi
