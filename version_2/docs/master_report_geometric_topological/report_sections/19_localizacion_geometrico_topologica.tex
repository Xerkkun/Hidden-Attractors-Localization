...
Esta sección convierte el lenguaje geométrico de las secciones anteriores en
un método de búsqueda. Su propósito no es reemplazar la función descriptiva,
la continuación o los puntos perpetuos, sino proporcionarles un atlas común:
...
\ifHAFOPedagogy
\subsection{Qué se necesita saber antes y en qué orden estudiarlo}
\label{subsec:gt-prerequisitos}

Complete las etapas 1--7 de \cref{sec:programa-estudio-bibliografico}. Antes de
usar esta propuesta debe poder:
\begin{enumerate}
  \item calcular $Df$, $Df\,f$, espectros y derivadas de Lie;
  \item definir flujo, cuenca, frontera, sección y mapa de retorno;
  \item interpretar grado, homotopía y punto fijo sin confundir existencia con
        atracción;
  \item distinguir periodicidad, recurrencia, caos y ocultedad;
  \item para Caputo, derivar la ecuación de Volterra y declarar la historia.
\end{enumerate}
Las páginas y ejercicios exactos están únicamente en la ruta inicial para no
repetir inventarios bibliográficos dentro de la teoría.
\fi

\subsection{Ventajas, aportes y límites del enfoque}
\label{subsec:gt-ventajas}

El aporte operativo es convertir una búsqueda ciega en una búsqueda por
objetos matemáticos de dimensión menor y, después, someter esos objetos a una
cadena de pruebas cada vez más fuerte.
...
\end{equation}
La superficie distinta
\begin{equation}
 S_J=\{x:\det Df(x)=0\}
 \label{gt:eq:singular-jacobian-surface}
\end{equation}
se llamará \emph{superficie de Jacobiano singular}; no debe confundirse con
las superficies de velocidad o aceleración de \eqref{gt:eq:critical-surfaces}.
...
$S_J$. La propuesta publicada de usar $\mathcal C$ para localizar atractores
ocultos es un antecedente, no una contribución nueva de este reporte. Tampoco
se ha demostrado que toda curva conectante atraviese toda cuenca; se usa como
familia priorizada de condiciones iniciales.
...
\subsection{KCC/Jacobi: región geométrica de enfoque, no certificado de cuenca}
\label{subsec:gt-kcc}
...
\end{align}
En los equilibrios bloqueados del expediente, Wolfram obtiene
\begin{equation}
...
\subsection{Traslado correcto a Caputo}
...
los antecedentes consultados y se propone como parte del proyecto.
...
trabaja directamente con \eqref{gt:eq:caputo-volterra}; no se inventa una
...
filtro de semillas Caputo son una deducción del proyecto. No afirman una regla
de cadena fraccionaria para $F(x(t))$.

\subsubsection{Evento conectante dependiente de historia}

El carril global conserva la definición secuencial ya introducida:
\begin{equation}
 \mathcal A_q[x](t)
...
\end{equation}
al refinar paso y horizonte; un mínimo visual aislado no basta.
...
\cite{KhristenkoWohlmuth2023Rational}. Lo nuevo propuesto no es la elevación,
sino usarla como puente controlado hacia grafos de cajas e índice de Conley.
...
\subsection{Mapa de antecedentes, deducciones y novedad defendible}
\label{subsec:gt-novelty-map}

\begin{longtable}{@{}L{4.0cm}L{3.0cm}L{6.2cm}@{}}
\caption{Qué existía y qué añade esta formulación.}
...
se presenta aquí para fijar hipótesis; es una formalización del fundamento, no
...
\cite{RoyRayChowdhury2024KCC}. Aquí se integra como ranking y se explicitan
...
Ocultedad funcional frente a ocultedad de reinicio & Propuesta del proyecto &
...
$\mathcal A_{q,\nabla}$ & Propuesta del proyecto & Derivada de la expansión de
Volterra con $s=h^q$ y de geometría diferencial. Es un filtro de arranque, no
una órbita conectante global.\\
Evento conectante FPP--H sobre historias & Propuesta del proyecto & Usa la
...
de integración & La \cref{prop:gt-relative-hiddenness} combina herramientas
...
Banco unido DF--Machado--continuación--PP--superficies--CC--KCC & Aporte
metodológico & La novedad defendible es la integración causal, las reglas de
evidencia y la comparación con igual presupuesto, no cada herramienta por
separado.\\
\bottomrule
\end{longtable}

\begin{proposalbox}
\textbf{Regla de redacción de tesis.} Se usará ``antecedente'' para una
construcción publicada, ``deducción'' para una consecuencia demostrada aquí y
``propuesta'' para un protocolo todavía no cerrado dinámicamente. Ningún
resultado de Wolfram sobre $f$, $Df$ o $Df\,f$ se describirá como prueba de
atracción, caos, cuenca u ocultedad.
\end{proposalbox}
