...
El atlas de la \cref{sec:localizacion-geometrico-topologica} se aplica a los
campos fijados. Esta etapa proporciona ecuaciones exactas, raíces algebraicas,
...
\cref{sec:resultados-piloto-gt} presenta las integraciones de PLL,
...
\end{equation}
El residuo $JF$ se anula en ambos pares a la precisión de trabajo. El primer par fue
...
corta al umbral fijado, por lo que el resultado establece acotación a tiempo finito
...
\end{align}
La sección $x_3=0$ usada en el mapa de conmutación es a la vez
...
\end{equation}
Las integraciones llevan estos PP a los ciclos internos y las semillas de
función descriptiva a la rama exterior. El control de horizonte no alcanza
el umbral fijado: la deriva entre las dos ventanas más largas es
$26.24\%$ para los PP y $10.69\%$ para las semillas armónicas.
\ifHAFOBasinMaterial
Además, el bracket entre
ambas rutas encuentra un tercer destino y no define una separatriz binaria
(\cref{sec:resultados-piloto-gt}). Los PP localizan un destino distinto de la
rama exterior; para explorar esa cuenca se necesitan otras semillas sobre
las superficies, las ramas de $\mathcal C$ o el borde entre ciclos.
\fi
La simetría $X\mapsto-X$ reduce el banco a un hemisferio;
...
cubierta $\theta\mapsto\theta+2\pi k$.
En las integraciones comparadas, $p_+$ converge de manera
robusta al foco, mientras $p_-$ permanece como transitorio acotado
sin destino resuelto.
\ifHAFOBasinMaterial
El intervalo de búsqueda no define un borde entre cuencas porque sus
extremos no tienen dos destinos distintos clasificados.
\fi
El ciclo silla y el ciclo estable ya localizados continúan siendo controles para una ventana
...
El sistema $A=0$ tiene cinco raíces reales: tres son los equilibrios y las dos restantes
son los PP. La eliminación, el descarte de denominadores y el conteo de
raíces se desarrollan en \cref{eq:pp-lorenz-reduced-three,eq:pp-lorenz-eliminant};
su reconstrucción da
\begin{equation}
...
reproducción dinámica local. La \cref{prop:geofo-lorenz-boundedness} demuestra
no explosión y un elipsoide absorbente para los reinicios de este modelo; la
\cref{prop:geofo-profile-precompact} aporta precompacidad de sus perfiles.
Identificar el destino de estas semillas y una frontera entre $E_+$, $E_-$ y
el candidato requiere todavía reproducir la dinámica y fijar la clase de
perfiles admisibles.
...
PP. La separación de $x=0$, $z=0$ y $z\ne0$, junto con los resultantes de
\cref{eq:pp-rf-z0-resultant,eq:pp-rf-eliminant}, demuestra ese conteo.
Cuatro representantes son
\begin{align}
...
\end{equation}
Las raíces refinadas tienen residuos menores que $10^{-40}$ y pertenecen a $S_J$. Para
...
\subsection{Muñoz--Pacheco: cuatro raíces suaves sin equilibrios}
...
están separadas de $x=0$, por lo que el cálculo regional es válido. En la
frontera, la recta $(0,y,0)$ satisface el criterio direccional, pero no define
PP clásicos; sus trayectorias exactas de \cref{eq:pp-munoz-boundary-orbit}
son no acotadas. La simetría
es $(x,y,z)\mapsto(-x,y,-z)$. Para $q=0.97$ y $0.996$ estas raíces son semillas
Picard--Caputo. En ausencia de equilibrios, el análisis se centra en
localización, coexistencia, aislamiento y dependencia de la historia; esa
ausencia no aporta evidencia de caos.

\subsection{Estructura algebraica y clasificación dinámica}

\begin{longtable}{@{}L{3.1cm}L{3.0cm}L{3.0cm}L{4.2cm}@{}}
\caption{Estructura geométrica y problemas dinámicos por resolver.}
...
convergencia entre integradores sin establecer.\\
...
MAVPD & Dos PP exactos & Central &
\ifHAFOBasinMaterial
PP interiores y DF exterior; tercer destino en el intervalo de búsqueda.
\else
Semillas PP y DF disponibles para comparación dinámica.
\fi\\
PLL & Dos PP sobre círculos singulares & Cubierta angular &
\ifHAFOBasinMaterial
$p_+$ llega al foco; $p_-$ y el borde quedan sin resolver.
\else
PP y retorno disponibles para comparación dinámica en el cilindro.
\fi\\
Lorenz $q=0.995$ & Dos FPP--A; cota absorbente para reinicios & $(-x,-y,z)$ &
¿Qué destino alcanzan y qué borde relativo separa $E_\pm$ del candidato?\\
RF $q=0.998$ & Ocho FPP--A, cuatro clases & $(-x,-y,z)$ & ¿Qué semillas son
acotadas, recurrentes y distintas de los cinco equilibrios?\\
Muñoz $q=0.97/0.996$ & Cuatro FPP--A suaves; recta direccional no acotada & $(-x,y,-z)$ & ¿Cómo se separan
