\section{Campaña dinámica geométrico--topológica: de la semilla a la certificación}
\label{sec:campana-gt}

Las secciones anteriores ya establecieron la geometría de flujo para \(q=1\),
la formulación funcional apropiada para Caputo, la construcción armónica, la
continuación y el papel limitado de los puntos perpetuos. Aquí no se repite esa
teoría. Se convierte en una \emph{campaña experimental preregistrada} para los
casos del reporte. Su propósito es contestar, en orden, cuatro preguntas que
...
\begin{hallazgo}{Estado de los resultados de esta sección}
La primera ronda B0--B2 ya fue ejecutada para PLL, MAVPD y Wu, con 24 semillas,
...
decisiones de ocultedad permanecen \textbf{pendientes}. Ninguna tabla declara
que se haya encontrado un atractor nuevo, caótico u oculto.
\end{hallazgo}

\ifHAFOdigitalcontent
\begin{hallazgo}{Primera capa de software disponible}
Ya se implementó una primera versión experimental del motor geométrico, el
banco unificado de semillas, el clasificador conservador de destinos y el
seguimiento de bordes. El motor calcula por separado \(f(x)\), \(J_f(x)\) y
...
el contrato entero o Caputo, el terminal inferior y la referencia de historia.
El clasificador distingue equilibrio, movimiento casi periódico proyectado,
recurrencia acotada, escape, transitorio y ambigüedad. La primera capa del
seguimiento de bordes exige destinos terminales distintos, confirmados con más
...
la trayectoria prolongada de borde exigida por los pasos 5--6 de este capítulo.

Esta disponibilidad hizo posible la primera ejecución controlada, pero no
completa TG0--TG8. Las pruebas unitarias verifican contratos y casos sintéticos,
Wolfram verifica las identidades algebraicas declaradas y el piloto compara
...
La evidencia de campaña se inicia, sin fabricar resultados, en
\ruta{version_2/validation/06_geometric_topological_campaign/}.
\end{hallazgo}
\fi

\subsection{Contrato común: qué sistema y qué espacio se están probando}
...
\end{equation}
mientras que el carril fraccionario conmensurable fijará explícitamente
\begin{equation}
 {}^{\mathrm C}D_{t_0+}^{q}x(t)=f(x(t);\mu),
 \qquad 0<q<1,\qquad x(t_0)=x_0.
 \label{camp:eq-caputo-contract}
\end{equation}
Si un caso usa órdenes no conmensurables, el contrato almacenará cada
\(q_i\); no se sustituirá ese sistema por uno con un orden promedio.
...
truncamiento formarán parte del contrato, no se elegirán después de ver el
resultado.
...
& La gran norma del campo en el par FPP--A lo convierte en control de una
  semilla algebraicamente correcta pero dinámicamente poco prometedora; la
  ruta armónica y la continuación deben competir con ella.
& El FPP--A sólo parece exitoso por un horizonte corto, o cualquier ruta
...
& El evento \(x=0\) no converge, la recurrencia desaparece o los dos órdenes
  sólo parecen distintos por la ventana temporal. Como no hay equilibrios, el
  rótulo usual de atractor oculto no se aplicará vacíamente.\\
\bottomrule
\end{longtable}
...
La campaña no permitirá que un método gane por disponer de más intentos. Se
construirá primero la unión etiquetada
\begin{equation}
 \mathcal S_0=
...
hará después de aplicar \cref{camp:eq-quotient-distance}; nunca se borrará la
etiqueta del método que produjo un representante coincidente.

\paragraph{Semillas armónicas.}
\(\mathcal S_{\mathrm{DF}}\) contendrá las soluciones de balance fundamental y,
cuando el residuo lo exija, las fases del balance multiharmónico. El carril de
...
rechazado, no como semilla promovida. Las fases \(0,\pi/2,\pi,3\pi/2\) se
...
estacionarias de \(J_f f=0\) admitidas por el dominio. En el carril Caputo se
interpretarán únicamente como cancelación del segundo coeficiente de
Picard--Caputo, según \cref{sec:perpetual-points}; no se llamarán puntos de
aceleración fraccionaria nula. Chua PWL y Kalman--Fitts son controles sin PP;
su banco no se rellenará con minimizadores numéricos que no satisfagan las
ecuaciones.
...
\paragraph{Ranking KCC, no certificación.}
...
\subsubsection*{TG0: fijar el objeto y congelar el contrato}
...
estados, a la sección de reinicio o al espacio funcional. Si falta uno de
estos datos, la campaña no comienza.

\subsubsection*{TG1: auditoría algebraica antes de integrar}
...
\emph{indeterminada}. Lorenz generalizado y RF no pasan a TG5 sin superar esta
compuerta.
...
\end{equation}
No se empalma el último punto de un archivo con un integrador Caputo nuevo y
se presenta ese empalme como memoria heredada. En campos PWL cada lado del
...
medidas empíricas, retornos y exponentes. Una etiqueta dinámica no se promueve
si depende de un único paso, integrador u horizonte.
...
centros de caja no es suficiente: las imágenes deben encerrarse con aritmética
...
Cada caso termina en una de cuatro decisiones: \emph{detener},
\emph{reformular}, \emph{conservar como candidato} o \emph{promover}. La
...
nominal de \(h\) sea fino en Chua y grueso en el PLL. Si el expediente ya tiene
...
B3 & \(2.5\times10^{-3}\) & \(400\) & \(2/3\) & sólo candidatos promovibles &
...
En Caputo el oráculo de memoria completa será ABM--PECE con suma desde \(t_0\).
...
El costo cuadrático del oráculo impide usarlo de manera indiscriminada en B3.
...
directa. La reducción de costo nunca autoriza a reducir también la longitud de
memoria sin declararlo.
...
\(\varepsilon_K^{(m)}\); no se descartará. También se medirá el error de
solución frente al oráculo de memoria completa en B0--B2. Los
...
\begin{enumerate}
  ...
  \item espectro de Lyapunov cuando el contrato variacional sea válido;
  \item prueba \(0--1\), entropía o complejidad sólo como diagnósticos
        complementarios, nunca como sustitutos de convergencia;
...
compatible con la memoria. Un exponente máximo positivo se promoverá sólo si
converge con el paso, la ventana, el algoritmo QR y el modelo de memoria.
...
cantidad anterior no acotada. Los artefactos originales de la campaña,
producidos con el esquema 1.0, permanecen sellados y se interpretan con su
semántica original; las corridas nuevas usan el esquema 1.1. La corrección no
...
empíricas. La coincidencia visual no basta. Si los tres indicadores discrepan,
el destino será \emph{ambiguo} aunque la proyección se parezca a una figura ya
publicada.
...
propiedades requieren compuertas adicionales.
...
\subsection{Reglas de detener, reformular, conservar o promover}
\label{camp:decision-rules}

\paragraph{Detener una rama.}
Se detiene si el sistema viola el contrato, si el residuo algebraico no baja
...
Detener una rama no autoriza a descartar las otras ramas de
\cref{camp:eq-seed-union}.
...
la métrica correcta. La reformulación puede cambiar escala, sección o dominio,
pero debe generar un contrato nuevo; no se edita retrospectivamente el
anterior.
...
aislante ni exclusión rigurosa de cuencas de equilibrio. La palabra permitida
es \emph{candidato}; quedan prohibidas \emph{atractor oculto demostrado} y
\emph{caos certificado}.

\paragraph{Promover.}
La promoción dinámica requiere B3, observables convergentes, perturbaciones
que regresan y una frontera reproducible. La promoción topológica requiere
además TG7. En Caputo toda promoción indicará si concierne a la sección reset,
a una clase de historias o al lift aproximado. Una propiedad demostrada para
el lift no se transfiere por parecido visual.

\subsection{Escala de evidencia EV--TG0--EV--TG7}
\label{camp:evidence-levels}

Para no confundirla con las etapas TG0--TG8, en los expedientes cada nivel se
...
equilibrios, EV--TG6 no se obtiene de manera vacía: se informa coexistencia,
accesibilidad y frontera, y se declara que la definición basada en cuencas de
equilibrio no discrimina ese caso.
...
\caption{Orden de ejecución y compuerta mínima. La última columna distingue la
primera ronda ejecutada de los pasos que siguen pendientes.}
...
& Simetría central; no usar periodicidad exacta.
...
MAVPD, \(\xi=3.5\)
& Mismo banco transportado sin retocar cuotas.
...
La primera ronda ya cubrió MAVPD $\xi=3.1$, PLL y Wu. Para continuar detectando
...
Este orden no expresa una jerarquía de importancia científica. Reduce el
riesgo de gastar el mayor presupuesto en un caso cuando aún no se ha probado
que el detector de eventos, el clasificador o la representación de memoria
funcionan en controles conocidos.

\ifHAFOdigitalcontent
\subsection{Trazado digital}
\label{camp:digital-trace}

La edición digital conserva, sin incorporar listados de código, la primera
ficha ejecutada bajo
...
Cada registro enlazará modelo, semilla, contrato, integrador, historia, ventana,
clasificador y nivel EV--TG, sin información administrativa ajena al protocolo.
\fi

\begin{proposalbox}
\textbf{Regla final de lectura.} La función descriptiva, Machado, la
continuación, PP/FPP, superficies críticas, curvas conectantes, ranking KCC,
edge tracking y métodos orientados a conjuntos no compiten por ser el único
método de localización. Forman una cadena de costo y fuerza crecientes. Los
primeros proponen semillas; la integración causal descarta incompatibilidades;
la geometría de cuenca organiza destinos; y la topología validada, cuando sea
posible, establece qué parte de esa organización puede sostener una
afirmación matemática. La primera ronda aporta resultados dinámicos finitos
EV--TG2/3, pero los niveles EV--TG4 y EV--TG7, la topología certificada y la
ocultedad global siguen siendo hipótesis de trabajo, no resultados.
\end{proposalbox}
