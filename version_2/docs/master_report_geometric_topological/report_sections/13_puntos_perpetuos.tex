...
La revisión del corpus bibliográfico local ---artículos descargados y
colección de Zotero--- recuperó trabajos directos sobre puntos perpetuos y sus
límites \cite{DudkowskiPrasadKapitaniak2015,NazarimehrSaediJafari2017,
NagChowdhuryGhosh2020}, además de revisiones generales de atractores ocultos y
métodos de localización \cite{DudkowskiEtAl2016,GuanXie2025}, pero no localizó
una publicación primaria que formule puntos perpetuos para sistemas
de Caputo preservando explícitamente la memoria, la falta de regla de cadena
local y la ausencia general de semigrupo. Este resultado acota el corpus
consultado; no demuestra inexistencia bibliográfica universal. La extensión
fraccionaria que sigue se formula, en consecuencia, como una
\emph{propuesta metodológica derivada de la formulación de Caputo}, no como una
teoría ya establecida.

\begin{table}[H]
\centering
\caption{Procedencia bibliográfica y alcance matemático de las construcciones.}
\label{tab:pp-status-map}
\small
\begin{tabularx}{\linewidth}{@{}L{0.30\linewidth}Y@{}}
\toprule
Construcción & Procedencia y significado \\
\midrule
...
FPP--A & Formulación propuesta: cancela el término local de orden
\(h^{2q}\); no es aceleración fraccionaria global. \\
FPP--H & Formulación propuesta: evento dependiente de la historia, no punto
intrínseco de \(\mathbb R^n\). \\
Caso inconmensurable y versión Bouligand/PWA & Propuestas teóricas que exigen
...
\end{equation}
La separación en la sección de reinicio es más débil que la separación en todo
el espacio funcional. Los sondeos realizados certifican únicamente la convención de
...
prueba de regresión necesaria para una implementación general, porque evita
suponer que todo punto perpetuo localiza el atractor de interés.
...
divergencia y de semillas inadecuadas & Wolfram; requiere validación adicional.\\
Kalman--Fitts & Ninguna, porque \(\det J_f=0.98191881\) & Control negativo de
la condición singular necesaria & Wolfram y análisis A1 existente.\\
MAVPD & Dos raíces cerradas por parámetro & Experimento preliminar entero & Wolfram,
...
Muñoz--Pacheco \(q=0.97\) & Cuatro raíces cerradas & Experimento preliminar Caputo
...
\subsubsection{Muñoz--Pacheco: cuatro raíces cerradas y experimento preliminar Caputo}
...
prueba de que las cuatro semillas generan órbitas computables y un caso de referencia para
...
metodología A1 del análisis son, de nuevo, complementos indispensables.
...
\subsection{Procedencia bibliográfica y alcance}

\begin{proposalbox}
\textbf{Resultados establecidos.} La definición \(J_f f=0\), su uso como semilla y
sus limitaciones pertenecen a la literatura de puntos perpetuos. Caputo,
Volterra, Picard y L1 son herramientas establecidas.

\textbf{Formulaciones evaluadas.} (i) reinterpretar \(J_f f=0\) para
...
La formulación completa no se identificó en el corpus revisado. Su prioridad
histórica permanece indeterminada y requiere una revisión sistemática
adicional, además de teoremas de existencia y regularidad para
\cref{eq:fpph-history-integral}.
\end{proposalbox}

La evaluación numérica propuesta comienza con MAVPD \(q=1\) como control
positivo y con Chua PWL y Kalman como controles sin PP. Muñoz--Pacheco
\(q=0.97\) constituye el primer caso de referencia FPP--A/FPP--H; Lorenz
\(q=0.995\) y RF \(q=0.998\) proporcionan las siguientes pruebas MIMO; Wu
\(q=0.99\) actúa como control fraccionario con análisis existente; y c590,
como prueba de rechazo de semillas lejanas.
