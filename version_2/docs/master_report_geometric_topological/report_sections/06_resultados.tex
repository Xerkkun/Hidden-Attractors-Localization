...
Las reconstrucciones bibliográficas proporcionan los controles para evaluar
los casos localizados mediante la metodología propuesta. Los sondeos
cuantifican el contacto con cada conjunto candidato a distintas distancias
de los equilibrios.
...
\]
con residuo complejo de cierre \(9.93\times10^{-16}\). Con la configuración
EFORK--3 de referencia \((T=320,\ T_{\mathrm{burn}}=80)\) y su clasificador por
...
\subsubsection{Danca y Wu bajo sus protocolos reconstruidos}
...
protocolo ABM de \cref{subsec:protocolos_bibliograficos}.
...
\midrule
Leonov--Kuznetsov, configuración EFORK--3 de referencia
...
\caption{Trayectorias de los controles bibliográficos de Danca y Wu,
de la continuación no suave propuesta y del control con pendientes distintas.}
...
produjo una trayectoria postransitoria de apariencia periódica, con frecuencia dominante
...
\end{equation}
Los diagnósticos indican un régimen regular de apariencia periódica.
Las 36 perturbaciones alrededor de \(X_{\mathrm{end}}\), integradas con ABM,
\(h=0.01\) y memoria completa, hicieron contacto con la nube objetivo: 12 de
12 para cada radio \(10^{-6}\), \(10^{-4}\) y \(10^{-2}\). Son pruebas de
robustez respecto de la condición inicial a paso fijo. La convergencia de ese
control respecto de \(h\) requiere la comparación descrita en
\cref{sec:control_numerico}.
...
corresponden al protocolo local predeclarado \(r\leq0.01\), y 10\,200 a la
...
En el protocolo local se registraron cero contactos y cero fallos numéricos en
...
La extensión macroscópica, evaluada por separado del protocolo local, añadió 10\,200
sondas. Las 6\,600 correspondientes a \(r=0.03\) y \(r=0.1\) produjeron cero
contactos; al combinarlas con el protocolo local se obtiene el acumulado
...
\]
dentro de la extensión diagnóstica y fuera del alcance local predeclarado.
\fi

\begin{hafocommonfigure}[H]
\centering
\includegraphics[width=0.86\linewidth]{nonsmooth_first_harmonic_comparison_es}
\caption{Trayectoria y primer armónico del caso no suave. La comparación
armónica describe el carácter regular de la respuesta.}
\label{fig:diagnosticos_no_suave_paper07}
\end{hafocommonfigure}

\begin{hafobasinfigure}[H]
\centering
\includegraphics[width=0.86\linewidth]{nonsmooth_volumetric_probe_overview_es}
\caption{Sondas volumétricas representativas de las clases usadas en
\cref{tab:sondeo_no_suave_completo}.}
\label{fig:sondeos_no_suave_paper07}
\end{hafobasinfigure}
...
La evaluación variacional de los 30 mejores casos seleccionó \texttt{c590} con
\begin{equation}
...
Los parámetros finales son los de \cref{eq:parametros_c590}. Para hallar los
equilibrios se resuelven primero las dos últimas ecuaciones:
\[
 x-y+z=0,\qquad \beta y+\gamma z=0
 \quad\Longrightarrow\quad
 y=\frac{\gamma x}{\beta+\gamma},\qquad
 z=-\frac{\beta x}{\beta+\gamma}.
\]
Al sustituir en la primera y usar
\(h(x)=a_1x+a_2\arctan(\rho x)\), queda la ecuación escalar impar
\[
 \left(a_1+\frac{\beta}{\beta+\gamma}\right)x
           +a_2\arctan(\rho x)=0.
\]
El origen es una raíz; las raíces no nulas aparecen por pares de signos
opuestos. Su evaluación para los parámetros seleccionados da
\begin{equation}
E_0=(0,0,0),\qquad
E_\pm=(\pm4.6494055135,\ \pm0.0017970023,\ \mp4.6476085112).
\label{eq:equilibrios_c590_resultados}
\end{equation}
Para aplicar el criterio de Matignon se evalúa el Jacobiano en cada punto y
se calcula el margen angular
\[
 m_q(E)=\min_{\lambda\in\sigma(DF(E))}
        \left(|\operatorname{Arg}\lambda|-\frac{q\pi}{2}\right).
\]
Un margen positivo coloca todos los autovalores fuera del sector de
inestabilidad; un autovalor estrictamente dentro de ese sector da margen
negativo. El cálculo clasificó \(E_0\) como inestable y \(E_\pm\) como
estables, con margen \(7.42265\times10^{-4}\) para los equilibrios externos.
...
\end{equation}
En la evaluación de horizonte largo, las medianas remuestreadas fueron
\begin{equation}
...
El número de trayectorias se obtiene multiplicando por los tres equilibrios
las muestras de cada radio. El bloque local contiene
\(3(100+200+300+400+500+600+700)=8\,400\) sondas; los radios \(1\) y \(2\)
añaden \(3\cdot800=2\,400\) y \(3\cdot900=2\,700\), respectivamente.
El sondeo reúne 13\,500 trayectorias: 8\,400 en la partición local
\(r\leq0.3\) y 5\,100 en la evaluación macroscópica \(r\in\{1,2\}\). El primer
...
\begin{hafocommonfigure}[H]
\centering
\includegraphics[width=0.86\linewidth]{arctan_first_harmonic_comparison_es}
\caption{Trayectoria y primer armónico de \texttt{c590}. La comparación
armónica acompaña el filtro de aperiodicidad.}
\label{fig:diagnosticos_c590_paper07}
\end{hafocommonfigure}

\begin{hafobasinfigure}[H]
\centering
\includegraphics[width=0.86\linewidth]{arctan_spherical_probe_overview_es}
\caption{Nueve historias representativas de los sondeos superficiales hasta
\(r=1\). La \cref{tab:sondeo_c590_completo} incorpora además la evaluación
canónica en \(r=2\).}
\label{fig:sondeos_c590_paper07}
\end{hafobasinfigure}
...
Dentro de las bolas, horizontes y reglas de contacto del protocolo local
...
ocultedad más allá del protocolo local. Para \texttt{c590}, la compatibilidad
