...
\begin{bloqueestudio}{Ejercicios}
\textbf{Formalismo.} En un mismo sistema produzca semillas por dos rutas
entre DF, Machado, continuación, PP/FPP y borde. Compare residuos algebraicos,
contrato de memoria, destino y prueba de cuenca bajo el mismo presupuesto.

\textbf{Interpretación.} Determine qué aporta cada ruta y qué afirmación no
...
\begin{enumerate}[nosep]
  ...
  ocultedad?
  \item Para cada caso del reporte: ¿qué es previo, qué es propuesta del
...
\begin{itemize}[nosep]
  ...
  PDF~108--126.
  \item Das \cite{Das2020Kindergarten}: ramas de \(s^q\), PDF~41--47;
  inicialización y convolución, PDF~109 y 119--135; historia y Laplace,
  PDF~247--310; sistemas multivariables, PDF~333--375.
  \item Diethelm \cite{Diethelm2010AnalysisFDE}: existencia, unicidad,
  regularidad y estabilidad, pp.~85--166.
\end{itemize}
\end{bloqueestudio}
\begin{bloqueestudio}{Ejercicios}
...
puntual no es, en general, un mapa de estado.
\end{bloqueestudio}
\begin{bloqueestudio}{Lecturas complementarias}
...
\textbf{Integrador.} Obtenga dos semillas de ramas distintas, intégrelas con
el mismo contrato entero o Caputo y compare destino y contacto con las cuencas
conocidas.
\end{bloqueestudio}
\fi}
...
\begin{bloqueestudio}{Lecturas}
Diethelm \cite{Diethelm2010AnalysisFDE}: ABM, pp.~195--210; caso
multi-término, pp.~211--228. Predictor--corrector original
...
\cite{DudkowskiPrasadKapitaniak2017}; contraejemplos
\cite{NazarimehrSaediJafari2017,NagChowdhuryGhosh2020}.
\end{bloqueestudio}
\begin{bloqueestudio}{Ejercicios}
...
del reporte. Desde Volterra obtenga
\(h^{2q}Df(p)f(p)/\Gamma(2q+1)\) sin usar una regla de cadena falsa.
\end{bloqueestudio}
\fi}
...
\begin{bloqueestudio}{Lecturas}
Datseris--Parlitz \cite{datseris2022} para observables numéricos; entropía de
cuenca \cite{DazaEtAl2016BasinEntropy}; Wada \cite{DazaEtAl2015Wada}.
\end{bloqueestudio}
\begin{bloqueestudio}{Ejercicios}
...
\cite{LeonovKuznetsovVagaitsev2011,KuznetsovEtAl2017}; sistemas
...
\end{bloqueestudio}
\begin{bloqueestudio}{Ejercicios}
Elija un caso entero y uno Caputo. Formule para ambos una pregunta futura sobre
variedades, retorno o frontera de cuenca; escriba el objeto geométrico, el
espacio de fases correcto, la prueba numérica y la hipótesis que aún faltaría
demostrar.
\end{bloqueestudio}
\fi}
