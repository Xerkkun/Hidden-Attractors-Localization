\section[Función descriptiva fraccionaria de Machado]
{Función descriptiva fraccionaria de Machado y transferencia sesgada}
\label{sec:machado-fdf}

\HAFOCardMachado

Se distinguen dos operaciones: la incorporación de un desplazamiento continuo
en una realización de Lur'e y la elevación de una función descriptiva a un orden
real en el sentido propuesto por Machado \cite{Machado2015FDF}. Ninguna de las dos
...
el fundamental de \(\psi(\sigma_0+A\cos\vartheta)\). La expresión
``transferencia sesgada'' designa de forma abreviada el \emph{cierre con función
descriptiva sesgada}; no define un nuevo operador lineal.
...
operación constante cuando existe un equilibrio \(\bar x\) con
\(r^{\mathsf T}\bar x=\bar\sigma\). Para comprobarlo, ponga
\(\xi=x-\bar x\) y
\[
 \psi_{\mathrm{res}}(u)
 =\psi(\bar\sigma+u)-\psi(\bar\sigma)-\psi'(\bar\sigma)u.
\]
La cancelación \(P\bar x+b\psi(\bar\sigma)=0\) da
\( {}^{\mathrm C}D_{0+}^{q}\xi
=P_{\bar\sigma}\xi+b\psi_{\mathrm{res}}(r^{\mathsf T}\xi)\).
Si \(\bar x\) no es equilibrio, queda el término afín
\(P\bar x+b\psi(\bar\sigma)\), que debe conservarse.
La función descriptiva del residuo es la de la característica desplazada
menos \(\psi'(\bar\sigma)\); usar con esta transferencia una función
descriptiva que aún contenga esa pendiente contaría la contribución dos veces.
En una frontera PWA puede no existir la derivada clásica: se deben
analizar las pendientes regionales y los cruces o justificar un marco
de derivadas generalizadas. Elegir una pendiente sin esa justificación
no produce una linealización clásica única.
...
\end{equation}
Cuando la curva cruza \(( -\infty,0]\), la continuación requiere desenvolver la
fase, fijar una hoja y registrar los cruces del corte, en lugar de aplicar la
rama principal punto a punto.
...
\(N^\mu\) cambia las unidades. Con una escala positiva \(N_\star\) de las mismas
unidades se define
\begin{equation}
...
Esta escala también controla la equivalencia entre realizaciones. Para
\(a>0\), el cambio
\(\widetilde b=ab\), \(\widetilde\psi=\psi/a\) conserva el campo
\(PX+b\psi(r^{\mathsf T}X)\). La transferencia y el coeficiente físico
se transforman como
\[
 \widetilde W_q=aW_q,\qquad \widetilde N=N/a,
 \qquad \widetilde W_q\widetilde N=W_qN.
\]
Para conservar también el cierre auxiliar se transforma la escala mediante
\(\widetilde N_\star=N_\star/a\). Entonces
\begin{equation}
 \widetilde N_{\mu,\widetilde N_\star}
 =\frac{N_\star}{a}
 \left[\frac{N/a}{N_\star/a}\right]^\mu
 =\frac{N_{\mu,N_\star}}{a},
 \label{eq:machado-scale-covariance}
\end{equation}
y se mantiene
\(\widetilde W_q\widetilde N_{\mu,\widetilde N_\star}
=W_qN_{\mu,N_\star}\), con la misma hoja porque el cociente
normalizado no cambia. En cambio, una potencia sin escala da
\(\widetilde W_q\widetilde N^\mu=a^{1-\mu}W_qN^\mu\): para
\(\mu\ne1\), su resultado depende del reparto arbitrario de ganancia.
La especificación de \(N_\star\) forma parte del método y de la
comparación entre realizaciones.
...
\end{equation}
Ambos coinciden idénticamente para \(\mu=1\); para \(\mu\ne1\) no
son equivalentes en general, aunque pueden coincidir en valores particulares,
como \(N=N_\star\). Si \(N(A)>0\) es
...
\end{align}
Para \(N_\star=1\), se resuelve \(N(A)=k^{1/\mu}\). La evaluación incluye la
pertenencia del objetivo al rango de \(N\). Si \(N\) es diferenciable,
\(N'(A)\ne0\), y se mantienen fijos \(k\) y \(N_\star\), derivar
\(\mu\log[N(A)/N_\star]=\log(k/N_\star)\) proporciona
\begin{equation}
 \frac{dA}{d\mu}
 =-\frac{N(A)\log[N(A)/N_\star]}{\mu N'(A)},
 \label{eq:machado-sensitivity}
\end{equation}
El crecimiento de este cociente mide la sensibilidad de la amplitud a
\(\mu\). Cerca de \(N'(A)=0\) debe revisarse la invertibilidad de la
ecuación de amplitud; en \(N=0\) la formulación logarítmica deja de
estar definida. Si el numerador y el denominador se anulan a la vez,
la fórmula por sí sola no decide el condicionamiento.

El registro de cada candidato conserva por separado
\begin{align}
 \rho_{\mathrm{aux}}
...
\end{equation}
con hoja fijada, y el cierre sustituto
\begin{equation}
 1-W_q(\mathrm i\omega)N_{\mu,b,N_\star}(\sigma_0,A)=0.
 \label{eq:machado-biased-closure}
\end{equation}
El valor medio físico \(\Psi_0\), que entra en el equilibrio afín, conserva su
valor sin elevarse a \(\mu\). También se evalúa
\begin{equation}
 \rho_{\mathrm{fis},b}
 =\left|1-W_q(\mathrm i\omega)N_b(\sigma_0,A)\right|.
 \label{eq:machado-biased-physical-residual}
\end{equation}
Las evaluaciones sesgadas usan \(\mu=1\), correspondiente a la familia
sesgada clásica. La aplicación de \cref{eq:machado-biased-family} con
\(\mu\neq1\) requiere evaluar separadamente el cierre auxiliar y el físico.

\subsection[Chua PWL, q=0.9998]
{Chua PWL con \(q=0.9998\)}
...
\end{equation}
La derivación respecto de la amplitud da
\begin{equation}
...
En la rama \(\omega_0=2.0402860510795615\),
\begin{align}
 W_q(\mathrm i\omega_0)
 &=4.761387970782572-1.52\times10^{-13}\mathrm i,\\
 k&=0.21002279296212067.
\end{align}
La referencia clásica \(\mu=1\) da
\(A_{\mathrm{cl}}=5.851767785486093\). Para \(\mu=2\), se obtiene
\begin{equation}
...
\subsection{Condiciones de aplicabilidad y protocolo}
