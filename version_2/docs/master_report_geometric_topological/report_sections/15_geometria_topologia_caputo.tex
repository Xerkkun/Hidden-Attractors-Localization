...
\ifHAFOPedagogy
\paragraph{Antes de leer esta sección.}
Complete las etapas 6 y 7 de \cref{sec:programa-estudio-bibliografico}. Debe
poder transformar un PVI de Caputo en una ecuación de Volterra y explicar el
papel del terminal inferior. Si aún interpreta $x(t)$ como un estado de Markov,
vuelva a la lectura sobre no localidad y memoria antes de continuar.
\fi

Esta sección traduce el lenguaje geométrico de las ecuaciones diferenciales
ordinarias al problema fraccionario que se estudia en el reporte. El cambio
conceptual decisivo es el siguiente:
...
Poincaré, cuenca, invariancia, atractor, conjugación y punto perpetuo. El
propósito conceptual es que cada gráfico tridimensional de los casos estudiados pueda
leerse como lo que realmente es: una \emph{proyección} de una evolución
funcional, no necesariamente el retrato de fase completo.

\begin{hallazgo}{Convención epistemológica de la sección}
Se marcarán tres niveles. Los resultados sobre Volterra, semidinámicas
...
ocultos, es una adaptación metodológica de este proyecto. La certificación de
atractores compactos, variedades invariantes, entropía o herraduras en los
casos concretos permanece abierta.
\end{hallazgo}

\subsection{Recordatorio: por qué un flujo ordinario vive en el espacio de estados}
...
Para existencia global y continuidad de la semidinámica funcional que se
usará abajo es suficiente suponer que \(f\) es globalmente Lipschitz. En los
sistemas polinomiales del reporte, que sólo son localmente Lipschitz, debe
...
\subsection{El espacio de fase correcto y la semidinámica}
...
Debe precisarse el alcance de esta ampliación. Los perfiles constantes
...
\end{equation}
El retrato dibujado en el reporte es su proyección
\begin{equation}
 \mathcal O_x^+(m)=
...
\begin{definition}[ocultedad funcional: adaptación fuerte del proyecto]
...
\begin{proposalbox}
Las definiciones anteriores son una traducción conservadora del criterio
clásico de cuenca. No se presentan como una definición universal ya aceptada
de atractor oculto fraccionario. El nivel actualmente verificable por el
...
\cref{eq:geofo-functional-hiddenness} requiere un programa de análisis y
muestreo adicional.
\end{proposalbox}
...
\end{equation}
Esta igualdad fundamenta la transformación afín de atractores y cuencas usada
en el reporte.
...
\end{equation}
Éste es el nivel correcto para transportar invariancia, recurrencia y
propiedades topológicas. Un homeomorfismo puntual
...
inmersión ambiente. El Caputo componente a componente utilizado en el reporte
es una formulación extrínseca en \(\mathbb R^n\); no se reclama independencia
...
\subsection{Periodicidad exacta, recurrencia y vocabulario correcto}
...
persistentes o recurrencia muy fuerte. Conviene medir por separado
\begin{align}
...
\end{align}
Un \(R_x\) pequeño no obliga a que \(R_{\mathfrak C}\) sea pequeño. En el
reporte deben usarse expresiones como \emph{oscilación acotada},
...
La \cref{sec:perpetual-points} introdujo dos carriles que ahora pueden
...
\centering
\small
\caption{Lectura geométrica de FPP--A y FPP--H.}
...
El siguiente protocolo complementa, no reemplaza, los niveles L0--L6 del
reporte.
...
\begin{enumerate}[label=\textbf{C\arabic*.}]
 ...
 política de memoria. Una ventana truncada o un reinicio define otro contrato.
 ...
 \item \textbf{Conservar dos carriles.} Para cada estado candidato \(p\),
...
\end{equation}
No debe promediarse el orden ni reutilizarse un único conjunto de pesos para
todas las componentes.

\subsection{Ejemplo pedagógico completo: relajación escalar}
...
\end{itemize}
Es el banco mínimo recomendado para comprobar que una implementación distingue
correctamente continuación y reinicio antes de estudiar Chua.

\ifHAFOPedagogy
\subsection{Guía de estudio y ejercicios de comprobación}

Estos seis niveles son el orden \emph{interno} de este capítulo, una vez
alcanzada la etapa fraccionaria de la ruta única en
\cref{sec:programa-estudio-bibliografico}.
Conectan los conceptos abstractos con el reporte sin suponer de inicio teoría
avanzada de espacios funcionales.

\paragraph{Nivel 1: del campo al flujo.}
Para una ODE, demostrar la ley \(\varphi_{t+s}=\varphi_t\circ\varphi_s\) usando
unicidad. Identificar órbitas, equilibrios, conjuntos invariantes y cuencas en
MAVPD o PLL. Después localizar exactamente qué paso usa que el estado puntual
es completo.

\paragraph{Nivel 2: de Caputo a Volterra.}
Derivar \cref{eq:geofo-split-memory} y comprobar
\(m_t(0)=x(t)\). Para la relajación escalar, comparar numéricamente
\begin{equation*}
 E_q[-a(t+s)^q]
 \quad\text{y}\quad
 E_q(-at^q)E_q(-as^q).
\end{equation*}
La diferencia es la primera prueba de que el reinicio cambia el sistema
observado.

\paragraph{Nivel 3: topología funcional.}
Elegir dos perfiles que coincidan en \([0,1]\) pero difieran después de
\(\theta=2\), y estimar \(d_{\mathfrak C}\). Repetir moviendo la diferencia a
\(\theta=20\). Este ejercicio muestra cómo la topología uniforme en compactos
pondera ventanas cada vez más lejanas.

\paragraph{Nivel 4: geometría de cuencas.}
Para Chua, dibujar la inmersión \(\iota(\mathbb R^3)\) como una sección dentro
de \(\mathfrak C\). Explicar por qué una bola euclídea alrededor de un
equilibrio no es una bola funcional y construir perturbaciones de la forma
\cref{eq:geofo-history-probes}.

\paragraph{Nivel 5: retorno y recurrencia.}
Construir primero una sección de Poincaré en MAVPD entero. En el caso Caputo,
guardar para cada cruce tanto \(x(t_k)\) como un vector de memoria. Verificar si
dos cruces cercanos en estado tienen sucesores cercanos después de condicionar
por la memoria. Si no, el gráfico puntual es una relación y no un mapa.

\paragraph{Nivel 6: afirmación topológica.}
Antes de usar las palabras \emph{atractor}, \emph{ciclo}, \emph{separatriz},
\emph{entropía} o \emph{conjugación}, escribir explícitamente:
\begin{enumerate}[label=(\alph*)]
 \item el espacio en el que vive el objeto;
 \item la semidinámica u operador respecto del cual es invariante;
 \item la topología o métrica usada;
 \item la evidencia analítica y la evidencia numérica disponibles.
\end{enumerate}
Si falta uno de esos cuatro elementos, la afirmación se conserva como
diagnóstico o conjetura.
\fi

\subsection{Mapa de resultados establecidos, adaptaciones y problemas abiertos}

\begin{longtable}{@{}L{0.22\linewidth}L{0.34\linewidth}L{0.36\linewidth}@{}}
\caption{Frontera de teoría y novedad en la reformulación geométrica Caputo.}
...
Derivado aquí de la formulación establecida; no se reclama como teorema nuevo.\\
Adaptación del proyecto & Jerarquía entre cuenca funcional y cuenca de
reinicio; definición operacional de ocultedad funcional; protocolo C1--C10;
lectura geométrica complementaria de función descriptiva, continuación y
FPP--A/FPP--H. &
Propuesta metodológica que debe contrastarse con más literatura y ejemplos.\\
...
\begin{hallazgo}{Síntesis geométrica para leer el reporte}
...
En campos afines globales, la linealidad de Caputo hace coincidir ambos
carriles. En Chua PWL, esa coincidencia sólo es local mientras toda la memoria
...
\subsection{Lectura caso por caso de los ejemplos fraccionarios}
...
Estos casos son, por ello, los bancos más limpios del reporte para construir
una semidinámica global sobre \(\mathfrak C\).
...
Lipschitz. Para promover sus nubes numéricas a atractores funcionales se
necesitan dos pasos que esta formulación todavía no contiene:
\begin{enumerate}
 ...
 funcional apropiado.
\end{enumerate}
La verificación de equilibrios, espectros de Matignon y realizaciones de Lur'e
de la \cref{part:nonchua} es álgebra local correcta, pero no completa esos dos
...
\end{itemize}
Al no haber equilibrios, el próximo objetivo geométrico debe ser construir y
comparar cuencas de diferentes perfiles de reinicio, no aplicar de forma vacía
el criterio clásico de vecindades de equilibrios.

\begin{table}[ht]
\centering
\small
\caption{Objeto geométrico correcto y siguiente prueba por familia.}
...
\subsection{Cierre: cómo cambia la pregunta geométrica}
...
atracción y ocultedad en sentido dinámico. El puente entre ambas es la
evaluación \(\operatorname{ev}_0\), y el puente desde una condición inicial
convencional es la inmersión \(\iota\).

Por ello, una profundización posterior no consiste en descartar los métodos ya
usados, sino en elevar sus resultados al espacio correcto: las semillas de
función descriptiva y FPP--A se conservan; la continuación debe preservar
