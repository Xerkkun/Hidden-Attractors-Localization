...
| tests/test_pypi_publication_status.py | Verificar pypi publication status | release_readiness | fast | unit | ninguna | no | no | conservar | Sincroniza manifiesto y docs con el paquete publicado en PyPI. || tests/test_figure_tests_do_not_write_real_library_figures.py | Verificar no escritura de tests en library_figures | hygiene | fast | unit | ninguna | no | no | conservar | Enforza el uso de tmp_path y monkeypatch en tests. |
