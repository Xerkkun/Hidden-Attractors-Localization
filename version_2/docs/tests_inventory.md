...
| tests/test_biased_chua_extended_stop_policy.py | Verificar que el plan extendido agrupe por radio y mantenga semillas reproducibles, condición estructural del corte tras el primer radio con contactos | validation_contract | fast | contract | ninguna | no | no | conservar | Protege el orden causal por radio y la asignación determinista de semillas para los tres equilibrios. |
...
| tests/test_c590_search_provenance.py | Verificar la reconstrucción exacta de los bancos PCG64, índices, parámetros y semillas global, c590 y seed9 | scientific_contract | fast | regression | ninguna | no | no | conservar | Fija la procedencia reproducible de 2,400 casos globales, 1,000 perturbaciones locales y la semilla fraccionaria seleccionada. |
...
| tests/test_rerun_c590_discovery.py | Verificar el plan ejecutable y los bancos exactos de la reconstrucción c590 | scientific_contract | fast | regression | ninguna | no | no | conservar | Protege las etapas global, local, variacional y Caputo, junto con la selección reproducible de c590 y seed9. |
...
| tests/test_pypi_publication_status.py | Verificar pypi publication status | release_readiness | fast | unit | ninguna | no | no | conservar | Sincroniza manifiesto y docs con el paquete publicado en PyPI. |
