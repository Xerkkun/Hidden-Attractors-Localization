...
| tests/test_cpc_readiness_metadata.py | Verificar metadatos y esqueleto CPC | hygiene | fast | unit | ninguna | no | no | conservar | Valida archivos minimos CPC y DOI archivado. |
...
| tests/test_cpc_submission_samples.py | Verificar sample input/output CPC | hygiene | fast | unit | ninguna | no | no | conservar | Valida que los samples existen y estan referenciados por archive_manifest.json. |
| tests/test_cpc_no_mojibake.py | Verificar ausencia de mojibake en archivos CPC principales | hygiene | fast | unit | ninguna | no | no | conservar | Evita regresiones de codificacion en metadatos, paper, docs y cpc_submission. |
| tests/test_cpc_known_remaining_work.py | Verificar pendientes legítimos CPC y modos strict/submission-strict | hygiene,cpc_readiness | fast | unit | ninguna | no | no | conservar | Valida que los pendientes finales declarados no bloquean readiness de repositorio/software. |
| tests/test_no_absolute_paths_in_promoted_evidence.py | Verificar ausencia de rutas absolutas en evidencia promovida | hygiene,cpc_readiness | fast | unit | ninguna | no | no | conservar | Escanea JSON/MD/TEX/BIB promovidos y permite solo secciones policy/legacy justificadas. |
