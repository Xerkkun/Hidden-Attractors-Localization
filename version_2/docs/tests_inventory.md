...
| tests/helpers/test_documentation_text.py | Funciones auxiliares para pruebas de documentación | hygiene | fast | unit | ninguna | no | no | conservar | Helper común de parsing de texto para tests. || tests/test_cpc_readiness_metadata.py | Verificar metadatos y esqueleto CPC | hygiene | fast | unit | ninguna | no | no | conservar | Valida archivos minimos CPC y DOI archivado. |
