...
En la librería, la función \texttt{rhs\_nonsmooth} no integra el sistema. Solo evalúa el lado derecho:
...
from hidden_attractors.models import equilibria_nonsmooth
\end{lstlisting}
...
for name, point in equilibria_nonsmooth(params).items():
    residual_norm = norm(rhs_nonsmooth(point, params))
...
params = chua_nonsmooth_parameters()
for name, eq in equilibria_nonsmooth(params).items():
    print(name, eq, np.linalg.norm(rhs_nonsmooth(eq, params)))
\end{lstlisting}
