...
La librería separa el campo vectorial del contrato fraccionario. El campo \(F(X)\) se evalúa en Python mediante funciones como \codepath{rhs_nonsmooth}; el orden \(q\), el paso \(h\), la memoria \(L_m\), el tiempo final \(T\) y el burn-in pertenecen al integrador o workflow.
...
p = chua_nonsmooth_parameters()
eqs = equilibria_nonsmooth(p)

for name, point in eqs.items():
    residual = np.linalg.norm(rhs_nonsmooth(point, p))
...
\subsection[ComparaciÃ³n clÃ¡sica contra Machado: df\_compare]{ComparaciÃ³n clÃ¡sica contra Machado: \codepath{df_compare}}
...
\begin{lstlisting}
hidden-attractors-unified-chua --run-mode machado_sweep --model nonsmooth --output-dir runs_machado_sweep --q 0.99 --h 0.01 --memory-length 8 --t-transient 80 --t-keep 100
\end{lstlisting}
...
\begin{lstlisting}
hidden-attractors-unified-chua --run-mode machado_sweep_fast --model nonsmooth --output-dir runs_machado_sweep_fast --q 0.99 --h 0.01 --memory-length 8 --t-transient 80 --t-keep 100
\end{lstlisting}
...
\neq\text{semilla armÃ³nica tipo Weyl}.
\end{gathered}
\end{equation}
...
\subsection[CÃ³mo leer sphere\_decision.csv]{CÃ³mo leer \codepath{sphere_decision.csv}}
...
ExportaciÃ³n TISEAN & \codepath{hidden-attractors-unified-chua}\newline\codepath{--tisean} & Series exportadas para anÃ¡lisis externo.\\
Lyapunov operativo & \codepath{hidden-attractors-unified-chua}\newline\codepath{--lyapunov} & Resumen de exponentes y convergencia.\\
Lyapunov estricto & \codepath{hidden-attractors-unified-chua}\newline\codepath{--lyapunov-strict} & Falla la corrida si el cÃ¡lculo falla.\\
