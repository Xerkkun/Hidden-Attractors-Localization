...
static inline double optional_double_arg(int argc,char **argv,const char *key,double default_value){ const char *s=find_arg(argc,argv,key); if(!s) return default_value; char *end=NULL; errno=0; double v=strtod(s,&end); if(errno||end==s||*end!='\0'){fprintf(stderr,"Valor invalido para %s: %s\n",key,s); exit(EXIT_FAILURE);} return v; }
