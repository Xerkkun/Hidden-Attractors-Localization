...
| **`CLAIM-CHUA-FRAC-REJECTED-001`** | El candidato oficial evaluado bajo el contrato de vecindades no se promueve como atractor oculto porque se detectó contacto autoexcitado. | Chua no suave / saturation nonlinearity / Candidate branch_0 | fraccionario, q = 0.9998 | `rechazado` | `validation/09_hiddenness_tests/hiddenness_tests_validation_summary.json` | `validation/09_hiddenness_tests/hiddenness_decisions.csv` | ninguna | Muestra el comportamiento conservador del filtro de ocultedad. Este caso se trata como referencia parcial / implementación parcial, y el candidato evaluado bajo contrato propio de la librería registró 1305 contactos autoexcitados, siendo consecuentemente rechazado. |
| **`CLAIM-CHUA-ARCTAN-FRAC-001`** | La metodología para el sistema de Chua fraccionario con no linealidad arctan se limita a una implementación algebraica parcial; la validación completa bajo el contrato completo de semillas, continuación, simulación y ocultedad permanece pendiente. | Chua fraccionario arctan | fraccionario, q < 1 (por ejemplo, q = 0.99) | `pendiente` | ninguna / pendiente | ninguna / pendiente | ninguna / pendiente | No existe promoción de atractor verificado como oculto para este caso de referencia. La biblioteca solo implementa el álgebra del modelo, Jacobian, y trayectoria ADM local, pero no una reproducción completa del atractor del paper. |
